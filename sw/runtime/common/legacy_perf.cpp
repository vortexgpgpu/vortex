...
extern int vx_dump_perf(vx_device_h hdevice, FILE *stream) {
  ...
  case VX_DCR_MPM_CLASS_DTCU: {
    ...
    perf_print(stream, "dtcu: compute=%" PRIu64 ", next_k_load_stall=%" PRIu64 ", tma_mem_wait=%" PRIu64 ", tma_buf_starve=%" PRIu64,
               compute, next_k_load_stall, tma_mem_wait, tma_buf_starve);
    perf_print(stream, "dtcu: tma_op_fill=%" PRIu64 ", tma_addrgen=%" PRIu64 ", tma_store_issue_stall=%" PRIu64 ", store_drain=%" PRIu64 ", smem_read_model=%" PRIu64,
               tma_op_fill, tma_addrgen, tma_store_issue_stall, store_drain, smem_read_model);
    perf_print(stream, "dtcu: next_tile_load_stall=%" PRIu64 ", prev_tile_store_stall=%" PRIu64,
               next_tile_load_stall, prev_tile_store_stall);
  } break;
