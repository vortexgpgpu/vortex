...
// The protocol uses ASCII characters: '0'-'7' encode TCK/TMS/TDI, 'R' reads TDO, 'r' resets, 'Q' quits.
// Stops processing when entering RUN_TEST_IDLE to allow the debug module to process state updates.
    ...
    if (recv_start < recv_end) {
      ...
      while (recv_start < recv_end) {
        ...
        if (!in_rti && tap->state() == RUN_TEST_IDLE) {
          entered_rti = true;
          // Call run_test_idle to allow debug module to process state updates
          // This is where the hart executes instructions when running continuously
          tap->run_test_idle();
          break;
        }
        in_rti = (tap->state() == RUN_TEST_IDLE);
      }
      unsigned sent = 0;
      while (sent < send_offset) {
        ssize_t bytes = write(client_fd, send_buf + sent, send_offset - sent);
        if (bytes == -1) {
