...
public:
  ...
  void write(uint32_t cid, uint32_t wid, uint32_t tid, 
    ...
    DT(2, "om-write: cid=" << std::dec << cid << ", wid=" << wid << ", tid=" << tid << ", x=" << x << ", y=" << y << ", backface=" << is_backface << ", color=0x" << std::hex << color << ", depth=0x" << depth);
...
void OMUnit::write(uint32_t cid, uint32_t wid, uint32_t tid, 
                    uint32_t x, uint32_t y, bool is_backface, uint32_t color, uint32_t depth, 
                    const CSRs& csrs, OMUnit::TraceData::Ptr trace_data) {
