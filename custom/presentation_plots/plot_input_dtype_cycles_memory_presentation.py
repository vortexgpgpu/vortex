...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create the combined presentation plot for run_b total cycles and memory requests."
    )
    parser.add_argument(
        "stats_dir",
        nargs="?",
    ...
    parser.add_argument(
        "-o",
        "--output",
        default=SCRIPT_DIR / "input_dtype_cycles_memory_presentation.png",
        type=Path,
        help="Output image path. A PDF copy is also written next to it.",
    )
    parser.add_argument(
        "--csv",
        default=SCRIPT_DIR / "input_dtype_cycles_memory_presentation.csv",
        type=Path,
...
def plot_cycles(ax, rows, run_b):
    ...
    baselines = {
        dtype: by_key[(dtype, run_b.VARIANT_ORDER[0])]["kernel_body_cycles"]
    ...
    for variant in run_b.VARIANT_ORDER:
        ...
        for dtype in run_b.DTYPE_ORDER:
            ...
            y_values.append(row["kernel_body_cycles"])
            dtypes.append(dtype)
...
def plot_presentation(rows, output, run_b):
    ...
    fig, (cycles_ax, memory_ax) = plt.subplots(
        2,
        1,
        figsize=(16.8, 15.8),
    ...
    plot_cycles(cycles_ax, rows, run_b)
    plot_memory(memory_ax, rows, run_b)
    ...
    fig.subplots_adjust(left=0.11, right=0.985, top=0.985, bottom=0.235, hspace=0.24)
