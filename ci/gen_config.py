...
def _compute_param_taint(toml_defs: Dict[str, Any], params_set: Set[str]) -> Set[str]:
  """Keys whose value (transitively) depends on a [[param]] symbol.

  Such configs cannot become localparams in a leaf package: their macro
  expansion references the [[param]] identifier, which is an RTL-derived
  localparam declared elsewhere (VX_gpu_pkg)."""
  refs: Dict[str, Set[str]] = {}
  for k, raw in toml_defs.items():
    deps: Set[str] = set()
    if _is_expr_string(raw):
      expr = _extract_expr(str(raw)) or ""
      deps |= set(_DOLLAR_IDENT_RE.findall(expr))
      deps |= set(_DOLLAR_BRACE_RE.findall(expr))
    refs[k] = deps
  taint: Set[str] = set(params_set)
  changed = True
  while changed:
    changed = False
    for k, deps in refs.items():
      if k not in taint and (deps & taint):
        taint.add(k)
        changed = True
  return taint
...
class MacroExprTranslator:
  ...
  def _name_ref(self, name: str) -> str:
    if self.d.kind == "sv" and name in self.params:
      ...
      # short, un-prefixed name (VX_config_pkg / VX_gpu_pkg convention); the
...
# -----------------------------
# SystemVerilog package emission ("sv_pkg" format)
# -----------------------------
# Emits a leaf package of localparams — one per integer value config — each
...
def _pkg_include(k: str, types: Dict[str, Any], params_set: Set[str],
  ...
  if k in params_set or k in taint:
    return False
  # only numeric configs become localparams; booleans are presence macros
  # (consumed by `ifdef) and strings are identifier macros — both stay as
  # macros and get no localparam. A float result is just a Python true-
  # division artifact (Verilog '/' is integer); the config is still numeric
  # and the emitted localparam defers to the macro for the real value.
  v = types.get(k)
...
def emit_sv_package(layout: Layout, enums: Dict[str, EnumSpec], types: Dict[str, Any],
                    params_set: Set[str], taint: Set[str],
                    output_path: Optional[str], config_basename: str) -> str:
  tool = os.path.basename(sys.argv[0])
  guard = _make_guard_macro(output_path, "VX_CONFIG_PKG")
  # package name is derived from the config TOML, not the output filename,
  # so it is stable regardless of where the file is written.
  pkgname = re.sub(r"\.[^.]*$", "", config_basename) + "_pkg"
  ...
  lines.append("")
  lines.append('`include "VX_define.vh"')
  lines.append("")
  lines.append("`IGNORE_UNUSED_BEGIN")
  lines.append("")
  lines.append(f"// Typed package view of {config_basename}: each integer `VX_CFG_*")
  lines.append("// value config resolved into a localparam. VX_gpu_pkg imports and")
  lines.append("// re-exports this package, so every RTL consumer sees these names.")
  lines.append(f"package {pkgname};")
  ...
  # enum-only keys (e.g. XLEN — declared via [[enum]], no section row)
  enum_only = [k for k in enums
               if k not in layout.ordered_keys
               and _pkg_include(k, types, params_set, taint)]
  if enum_only:
    lines.append("")
    lines.append("    // --- isa ---")
    for k in enum_only:
  ...
  for sec, keys in layout.sections:
    ...
    for k in keys:
      if k in emitted:
        continue
      if not _pkg_include(k, types, params_set, taint):
        continue
      emit_lp(sec_lines, k)
    if sec_lines:
      lines.append("")
  ...
  lines.append("")
  lines.append("`IGNORE_UNUSED_END")
  lines.append("")
...
def main(argv: List[str]) -> int:
  ...
  ap.add_argument("--format", "-f", choices=["cflags", "cpp", "verilog", "sv_pkg"], default="cflags")
  ...
  if args.format == "sv_pkg":
    # Resolve every key to classify its type: int -> localparam, bool/string
    # -> stays a macro. Integer enums are seeded so resolution can proceed
    # even though their value is build-supplied (-D); the *type* is invariant.
    seed = dict(overrides)
    for ek, espec in enums.items():
      if ek not in seed and all(isinstance(x, int) and not isinstance(x, bool)
                                for x in espec.values):
        seed[ek] = espec.values[0]
    r = Resolver(base=toml_defs, overrides=seed, enums=enums,
                 builtins=builtins, params=params_tbl)
    types: Dict[str, Any] = {}
    for k in list(layout.ordered_keys) + [e for e in enums if e not in layout.ordered_keys]:
      try:
        types[k] = r.resolve(k)
      except Exception:
        types[k] = None
    taint = _compute_param_taint(toml_defs, params_set)
    out = emit_sv_package(layout, enums, types, params_set, taint,
                          args.output, os.path.basename(args.config))
    if args.output:
