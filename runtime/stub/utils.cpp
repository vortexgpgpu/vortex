...
extern int vx_dump_perf(vx_device_h hdevice, FILE* stream) {
    ...
    case VX_DCR_MPM_CLASS_CORE: {
      ...
      // scoreboard stalls
      {
        ...
        if (num_cores > 1) {
          ...
          fprintf(stream, "PERF: core%d: scoreboard stalls=%ld (%d%%) (alu=%d%%, fpu=%d%%, lsu=%d%%, scrs=%d%%, wctl=%d%%, tex=%d%%, om=%d%%, raster=%d%%)\n"
          , core_id
  ...
  case VX_DCR_MPM_CLASS_CORE: {
    ...
    fprintf(stream, "PERF: scoreboard stalls=%ld (%d%%) (alu=%d%%, fpu=%d%%, lsu=%d%%, scrs=%d%%, wctl=%d%%, tex=%d%%, om=%d%%, raster=%d%%)\n"
      , scrb_stalls
      , scrb_percent
