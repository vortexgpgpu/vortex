...
inline bool quantize_mx_rowwise_blocks(OutT* quantized,
                                       ...
                                       bool per_col_scale,
                                       ConvertFn convert,
                                       uint8_t (*select_scale)(float)) {
  if ((rows == 0) || (cols == 0) || (block_size == 0)) {
    return false;
  }
  uint32_t kdim = per_col_scale ? rows : cols;
  if ((kdim % block_size) != 0) {
    return false;
  }

  uint32_t k_blocks = kdim / block_size;
  scale_meta.resize(per_col_scale ? (k_blocks * cols) : (rows * k_blocks));

  for (uint32_t major = 0; major < (per_col_scale ? cols : rows); ++major) {
    for (uint32_t kb = 0; kb < k_blocks; ++kb) {
      uint32_t k0 = kb * block_size;
      float max_abs = 0.0f;
      for (uint32_t i = 0; i < block_size; ++i) {
        uint32_t row = per_col_scale ? (k0 + i) : major;
        uint32_t col = per_col_scale ? major : (k0 + i);
        float v = dense[row * cols + col];
        max_abs = std::max(max_abs, std::abs(v));
      }
      uint8_t sf = select_scale(max_abs);
      if (per_col_scale) {
      ...
      for (uint32_t i = 0; i < block_size; ++i) {
        ...
        uint32_t col = per_col_scale ? major : (k0 + i);
        float v = dense[row * cols + col];
        quantized[row * cols + col] = convert(v, sf);
      }
    }
  }
...
template <typename OutT, typename ConvertFn>
inline bool quantize_mx_colwise_blocks(OutT* quantized,
                                       std::vector<uint8_t>& scale_meta,
                                       ...
                                       ConvertFn convert,
                                       uint8_t (*select_scale)(float)) {
  if ((K == 0) || (N == 0) || (block_size == 0) || ((K % block_size) != 0)) {
    return false;
  }

  uint32_t k_blocks = K / block_size;
  ...
  for (uint32_t n = 0; n < N; ++n) {
    for (uint32_t kb = 0; kb < k_blocks; ++kb) {
      ...
      float max_abs = 0.0f;
      for (uint32_t i = 0; i < block_size; ++i) {
        float v = dense_rowmajor[(k0 + i) * N + n];
        max_abs = std::max(max_abs, std::abs(v));
      }
      uint8_t sf = select_scale(max_abs);
      scale_meta[kb * N + n] = sf;
      for (uint32_t i = 0; i < block_size; ++i) {
        uint32_t k = k0 + i;
        quantized[n * K + k] = convert(dense_rowmajor[k * N + n], sf);
      }
    }
  }
...
inline bool quantize_mxfp8_a_rowmajor(uint8_t* quantized,
  ...
  return quantize_mx_rowwise_blocks<uint8_t>(
      quantized, scale_meta, dense, rows, cols, mxfp8::ele_block,
      false, convert, select_mxfp8_scale);
}

inline bool quantize_mxfp8_b_colmajor(uint8_t* quantized,
                                      std::vector<uint8_t>& scale_meta,
                                      const float* dense_rowmajor,
                                      uint32_t K,
                                      uint32_t N) {
...
inline uint8_t select_mxfp4_scale(float max_abs) {
  if (!(max_abs > 0.0f) || !std::isfinite(max_abs)) {
    return 127;
  }
...
inline bool quantize_mxfp4_rowwise_blocks(uint8_t* quantized,
                                          std::vector<uint8_t>& scale_meta,
                                          const float* dense,
                                          uint32_t rows,
                                          uint32_t cols,
  ...
  uint32_t kdim = per_col_scale ? rows : cols;
  if ((kdim % mxfp4::ele_block) != 0) {
    return false;
  }
  uint32_t packed_size = (rows * cols + 1) / 2;
  std::fill(quantized, quantized + packed_size, 0);
  uint32_t k_blocks = kdim / mxfp4::ele_block;
  scale_meta.resize(per_col_scale ? (k_blocks * cols) : (rows * k_blocks));

  for (uint32_t major = 0; major < (per_col_scale ? cols : rows); ++major) {
    for (uint32_t kb = 0; kb < k_blocks; ++kb) {
      uint32_t k0 = kb * mxfp4::ele_block;
      float max_abs = 0.0f;
      for (uint32_t i = 0; i < mxfp4::ele_block; ++i) {
        uint32_t row = per_col_scale ? (k0 + i) : major;
        uint32_t col = per_col_scale ? major : (k0 + i);
        max_abs = std::max(max_abs, std::abs(dense[row * cols + col]));
      }
      uint8_t sf = select_mxfp4_scale(max_abs);
      if (per_col_scale) {
        scale_meta[kb * cols + major] = sf;
      } else {
        scale_meta[major * k_blocks + kb] = sf;
      ...
      for (uint32_t i = 0; i < mxfp4::ele_block; ++i) {
        uint32_t row = per_col_scale ? (k0 + i) : major;
        uint32_t col = per_col_scale ? major : (k0 + i);
        uint8_t q = rv_ftomxfp4_s(bit_cast<uint32_t>(dense[row * cols + col]), sf, 0, nullptr) & 0x0f;
        detail::data_accessor_t<mxfp4>::write(quantized, row * cols + col, q);
      }
    }
  }
  return true;
...
inline bool quantize_mxfp4_colwise_blocks(uint8_t* quantized,
                                          std::vector<uint8_t>& scale_meta,
                                          const float* dense_rowmajor,
                                          uint32_t K,
                                          uint32_t N) {
  if ((K == 0) || (N == 0) || ((K % mxfp4::ele_block) != 0)) {
    return false;
  }
  uint32_t packed_size = (K * N + 1) / 2;
  std::fill(quantized, quantized + packed_size, 0);
  uint32_t k_blocks = K / mxfp4::ele_block;
  scale_meta.resize(k_blocks * N);
  for (uint32_t n = 0; n < N; ++n) {
    for (uint32_t kb = 0; kb < k_blocks; ++kb) {
      uint32_t k0 = kb * mxfp4::ele_block;
      float max_abs = 0.0f;
      for (uint32_t i = 0; i < mxfp4::ele_block; ++i) {
        max_abs = std::max(max_abs, std::abs(dense_rowmajor[(k0 + i) * N + n]));
      }
      uint8_t sf = select_mxfp4_scale(max_abs);
      scale_meta[kb * N + n] = sf;
      for (uint32_t i = 0; i < mxfp4::ele_block; ++i) {
        uint32_t k = k0 + i;
        uint8_t q = rv_ftomxfp4_s(bit_cast<uint32_t>(dense_rowmajor[k * N + n]), sf, 0, nullptr) & 0x0f;
        detail::data_accessor_t<mxfp4>::write(quantized, n * K + k, q);
      }
    }
  }
  return true;
...
inline bool quantize_mxfp4_a_rowmajor(uint8_t* quantized,
                                      std::vector<uint8_t>& scale_meta,
                                      const float* dense,
                                      uint32_t rows,
                                      uint32_t cols) {
  return quantize_mxfp4_rowwise_blocks(quantized, scale_meta, dense, rows, cols, false);
}

inline bool quantize_mxfp4_b_colmajor(uint8_t* quantized,
                                      std::vector<uint8_t>& scale_meta,
                                      const float* dense_rowmajor,
                                      uint32_t K,
                                      uint32_t N) {
  return quantize_mxfp4_colwise_blocks(quantized, scale_meta, dense_rowmajor, K, N);
}
...
inline bool quantize_mxint8_a_rowmajor(int8_t* quantized,
  ...
  return quantize_mx_rowwise_blocks<int8_t>(
      quantized, scale_meta, dense, rows, cols, mxint8::ele_block,
      false, convert, select_mxint8_scale);
}

inline bool quantize_mxint8_b_colmajor(int8_t* quantized,
                                       std::vector<uint8_t>& scale_meta,
                                       const float* dense_rowmajor,
                                       uint32_t K,
                                       uint32_t N) {
  ...
  return quantize_mx_colwise_blocks<int8_t>(
      quantized, scale_meta, dense_rowmajor, K, N, mxint8::ele_block,
      convert, select_mxint8_scale);
}
...
inline bool quantize_nvfp4_rowwise_blocks(uint8_t* quantized,
                                          std::vector<uint8_t>& scale_meta,
                                          float& tensor_scale,
                                          const float* dense,
                                          uint32_t rows,
  ...
  uint32_t k_blocks = kdim / nvfp4::ele_block;
  scale_meta.resize(per_col_scale ? (k_blocks * cols) : (rows * k_blocks));
  ...
  for (uint32_t major = 0; major < (per_col_scale ? cols : rows); ++major) {
    for (uint32_t kb = 0; kb < k_blocks; ++kb) {
      ...
      for (uint32_t i = 0; i < nvfp4::ele_block; ++i) {
        ...
        detail::data_accessor_t<nvfp4>::write(quantized, row * cols + col, q);
      }
    }
  }
...
inline bool quantize_nvfp4_colwise_blocks(uint8_t* quantized,
                                          std::vector<uint8_t>& scale_meta,
                                          float& tensor_scale,
                                          const float* dense_rowmajor,
                                          uint32_t K,
                                          uint32_t N) {
  if ((K == 0) || (N == 0) || ((K % nvfp4::ele_block) != 0)) {
    return false;
  }
  ...
  for (uint32_t n = 0; n < N; ++n) {
    for (uint32_t kb = 0; kb < k_blocks; ++kb) {
      ...
      float max_abs = 0.0f;
      for (uint32_t i = 0; i < nvfp4::ele_block; ++i) {
        float v = dense_rowmajor[(k0 + i) * N + n] / tensor_scale;
        max_abs = std::max(max_abs, std::abs(v));
      }
      uint8_t sf = select_nvfp4_block_scale(max_abs);
      scale_meta[kb * N + n] = sf;
      for (uint32_t i = 0; i < nvfp4::ele_block; ++i) {
        uint32_t k = k0 + i;
        float v = dense_rowmajor[k * N + n] / tensor_scale;
        uint8_t q = rv_ftonvfp4_s(bit_cast<uint32_t>(v), sf, 0, nullptr) & 0x0f;
        detail::data_accessor_t<nvfp4>::write(quantized, n * K + k, q);
      }
    }
  }
...
inline bool quantize_nvfp4_a_rowmajor(uint8_t* quantized,
                                      ...
                                      uint32_t rows,
                                      uint32_t cols) {
  return quantize_nvfp4_rowwise_blocks(quantized, scale_meta, tensor_scale,
                                       dense, rows, cols, false);
}

inline bool quantize_nvfp4_b_colmajor(uint8_t* quantized,
                                      std::vector<uint8_t>& scale_meta,
                                      float& tensor_scale,
                                      const float* dense_rowmajor,
                                      uint32_t K,
                                      uint32_t N) {
  return quantize_nvfp4_colwise_blocks(quantized, scale_meta, tensor_scale,
                                       dense_rowmajor, K, N);
}
