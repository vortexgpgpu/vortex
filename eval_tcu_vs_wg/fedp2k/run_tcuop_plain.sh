...
STATUS=$OUT/status_tcuop_plain.txt
cd "$BUILD"
: > "$STATUS"
...
echo "[$(date +%H:%M:%S)] START tcuop_plain" >> "$STATUS"
make -C tests/regression/sgemm_tcu_op clean >/dev/null 2>&1
if ! CONFIGS="$CFG" make -C tests/regression/sgemm_tcu_op > $OUT/tcuop_plain.log 2>&1; then
  echo "[$(date +%H:%M:%S)] BUILD-FAIL tcuop_plain" >> "$STATUS"; exit 1
fi
t0=$(date +%s)
CONFIGS="$CFG" ./ci/blackbox.sh --driver=rtlsim --app=sgemm_tcu_op \
  --args="-m 128 -n 128 -k 128 -s 0"   >> $OUT/tcuop_plain.log 2>&1
rc=$?; t1=$(date +%s)
echo "[$(date +%H:%M:%S)] DONE tcuop_plain rc=$rc wall=$((t1-t0))s" >> "$STATUS"
