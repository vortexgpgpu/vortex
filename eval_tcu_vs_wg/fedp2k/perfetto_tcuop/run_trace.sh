...
echo "[$(date +%H:%M:%S)] START trace" > $OUT/status.txt
make -C tests/regression/sgemm_tcu_op clean >/dev/null 2>&1
if ! CONFIGS="$CFG" make -C tests/regression/sgemm_tcu_op > $OUT/build.log 2>&1; then
  echo "[$(date +%H:%M:%S)] BUILD-FAIL" >> $OUT/status.txt; exit 1
fi
t0=$(date +%s)
CONFIGS="$CFG" timeout 10800 ./ci/blackbox.sh --driver=rtlsim --app=sgemm_tcu_op \
  --args="-m 128 -n 128 -k 128 -s 0" --perf=1 --debug=3 --log=$OUT/trace_iw4.log \
  > $OUT/blackbox.log 2>&1
rc=$?; t1=$(date +%s)
v=$(grep -aoE "PASSED|FAILED" $OUT/trace_iw4.log | tail -1)
[ $rc -eq 124 ] && v=TIMEOUT
c=$(grep -aoE "PERF: instrs=[0-9]+, cycles=[0-9]+" $OUT/trace_iw4.log | tail -1)
echo "[$(date +%H:%M:%S)] DONE rc=$rc wall=$((t1-t0))s ${v:-ABORT} ${c:-no-perf} size=$(du -h $OUT/trace_iw4.log | cut -f1)" >> $OUT/status.txt
