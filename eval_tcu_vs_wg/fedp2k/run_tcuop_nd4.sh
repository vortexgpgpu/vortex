...
CONFIGS="$CFG" timeout 1500 ./ci/blackbox.sh --driver=rtlsim --app=sgemm_tcu_op \
  --args="-m 64 -n 64 -k 128 -s 0"   >> $OUT/tcuop_nd4.log 2>&1
rc=$?; t1=$(date +%s)
echo "[$(date +%H:%M:%S)] DONE tcuop_nd4 rc=$rc wall=$((t1-t0))s" >> "$STATUS"
