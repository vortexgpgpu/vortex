# ThreadSplit (SCS) RTL Design Review

**Module:** [hw/rtl/core/VX_scheduler.sv](../../hw/rtl/core/VX_scheduler.sv) under
`VX_CFG_DIVERGE_TYPE_SCS` · **Config:** NT=8/NW=8, U55C, 300 MHz target
**Status:** validated (rtlsim 10/10; routed scheduler DUT closes 300 MHz)

## 1. What SCS is

SCS (schedulable convergence stack, "SCS") keeps the baseline IPDOM split/join model
...
stack) is retained — SCS is a *superset* of the baseline stack, not a replacement.
...
| **SCS** | **6,340** (1.58×) | 4,525 (+516) | 1 | +0.323 | 332 MHz |

SCS's forward-progress capability costs ~1.6× scheduler LUTs and one BRAM over the
...
- Bit-inert fences: a SCS build is code-identical to the pre-switch HEAD (mechanical
...
SCS is production-shaped: memory-first storage, baseline-shaped schedule path, 300 MHz
