...
| TSPLIT | 305.9 (**MET**) | +0.064 ns | 69,713 | 66,039 | 2,648 | 65,390 | 33 | 8 | 48 |
...
TSPLIT adds **+1,767 LUTs (+2.6%)** and **+276 FFs** to the core (plus one RAMB36 for the parked-split pool) and **meets 300 MHz** with the same margin as the baseline.

The ITS core is reported at two design points that bracket its area/timing tradeoff. **v1** is the reference-faithful design (all divergence state in flops): +12,108 LUTs (+17.8%) and misses 300 MHz at 228 MHz. **v3** is the memory-first redesign of §1.2 (per-thread PCs and barrier masks in LUTRAM, pipelined regroup engine): it cuts the core to **70,635 LUTs — within 900 LUTs of TSPLIT** and its FFs to near-baseline, but the pipelined min-PC regroup lands the core at **191 MHz** (the critical path is `s1_yielded → min-PC tree → grp_mask`, i.e. the ITS group formation itself).

**The Pareto picture is the headline.** On area-vs-Fmax, **TSPLIT dominates both ITS points simultaneously**: it is *smaller* than ITS-v1 (69.7K vs 80.1K) and *faster* than it (306 vs 228 MHz); and it *matches* memory-first ITS-v3's area (69.7K vs 70.6K) while running **60% faster** (306 vs 191 MHz). No storage strategy rescues ITS — flop-heavy trades area for timing headroom, memory-first trades timing for area, and neither reaches TSPLIT's corner, which achieves low area *and* 300 MHz closure at once. This is because TSPLIT keeps a baseline-shaped schedule path (priority-encode + register read) with one small BRAM pool, whereas ITS must regroup per-thread PCs through a min-PC tournament tree on every divergence event — a path that stays ~5 ns whether its operands come from flops or LUTRAM.
...
| TSPLIT | 332.2 (**MET**) | +0.323 ns | 6,340 | 4,525 | 334 | 1 | 0 | 1.58x |
...
LUTRAM (0 BRAM, 815 LUT-as-memory) — the same memory-first discipline TSPLIT applies to its pool.
...
**1.84×** baseline (from 4.33×), landing between TSPLIT's 1.58× and the old flop design — the
per-thread-PC and barrier storage now live in the right primitive. What remains is timing: even
memory-first and pipelined, ITS's divergence machinery is harder to close than TSPLIT's, because
TSPLIT keeps a baseline-shaped schedule path with one small BRAM pool while ITS must regroup
...
`vx_yield` sites** as compiled by POCL, yet still complete only under TSPLIT: the pred-park /
...
- **TSPLIT** — schedulable convergence stack + `vx_yield`.
- **ITS-CB** — ITS convergence barriers **only** (the reference / ablation, `VX_CFG_ITS_YIELD_DISABLE`).
...
| Kernel | Baseline | TSPLIT | ITS-CB | ITS+Y |
|---|---|---|---|---|
...
| Kernel | TSPLIT instrs | ITS+Y instrs | TSPLIT IPC | ITS+Y IPC |
|---|---|---|---|---|
...
**TSPLIT is cycle-invisible until forward progress is needed.** On all seven kernels that the
baseline completes, TSPLIT's instruction counts, cycle counts, and IPC are **identical to the
baseline** — not merely close. This follows from its design: on the convergent path TSPLIT's
...
**Forward progress: native in TSPLIT, bolt-on in ITS.** Bare convergence barriers (ITS-CB)
...
*not* a capability TSPLIT holds alone; it is a capability both architectures can reach. The
honest differentiator is **cost**: TSPLIT delivers it with one instruction (`vx_yield`) and a
...
largest scheduler of the four arms (Section 1). Notably ITS+Y is *faster* than TSPLIT on the
lock kernels themselves (lclist 84,979 vs 127,625 cycles; rangelock 24,305 vs 30,516) — per-thread
PCs let non-contending lanes advance while TSPLIT serializes the parked subgroups — so where locks
...
**Where ITS wins, and why.** ITS+Y runs `trig` ~40% faster than TSPLIT/baseline (1.22M vs 2.13M
...
lock kernels `lclist`/`rangelock` (non-contending lanes advance while TSPLIT serializes its parked
...
cycles** than TSPLIT: min-PC scheduling offers no *guarantee* of SIMD reconvergence — threads
...
scales with the warp width. TSPLIT, keeping the reconvergence stack, never fragments.

**Net assessment (both arms complete).** With ITS completed to ITS+Y, program coverage is a
tie: both TSPLIT and ITS+Y pass 10/10. TSPLIT still dominates on hardware cost (smaller
...
compatibility (baseline binaries run bit-identically under TSPLIT; ITS demotes split/join/pred to
...
fewer cycles). The paper's thesis therefore narrows honestly from "TSPLIT alone provides forward
progress" to **"TSPLIT provides forward progress at a fraction of ITS's area, timing, ISA, and
...
| Aspect | Baseline (IPDOM) | TSPLIT | ITS |
|---|---|---|---|
...
(`-mllvm -vortex-divergence-arch={ipdom|tsplit|its}`), selected automatically from
`VX_CFG_DIVERGE_TYPE` by the build system — no per-architecture compiler builds.

| Aspect | Baseline (ipdom) | TSPLIT (tsplit) | ITS (its) |
|---|---|---|---|
...
Columns: Baseline, TSPLIT, and the two ITS arms — ITS-CB (barriers only) and ITS+Y (completed).

| Capability | Baseline | TSPLIT | ITS-CB | ITS+Y |
|---|---|---|---|---|
...
Both split/join architectures are now feature-complete: TSPLIT and ITS+Y each pass 10/10. The
scoreboard, stated honestly across the six axes:

1. **Area** — TSPLIT: +2.6% core / 1.6× scheduler. ITS v1 (flop state): +17.8% core / 4.3×
   scheduler. ITS v3 (memory-first, §1): +4.0% core / 1.8× scheduler — the redesign brings ITS
   area to *within 900 core LUTs of TSPLIT*. Area alone is nearly a **tie** once ITS is built
   memory-first — but see axis 2, because that parity is bought with timing.
2. **Timing** — TSPLIT meets 300 MHz (+0.064 ns). ITS v1 misses at 228 MHz; ITS v3 misses worse
   at 191 MHz (the memory-first storage trades timing for the area win). **TSPLIT wins
   decisively, and the two axes together give the real result: on area-vs-Fmax, TSPLIT
   Pareto-dominates both ITS design points** — smaller than v1 and faster than both, matching v3's
   area at 60% higher frequency. No ITS storage strategy reaches TSPLIT's low-area/300-MHz corner.
3. **Runtime overhead** — TSPLIT is exactly zero vs baseline on every non-lock kernel; ITS+Y is
   ...
   **Split decision: TSPLIT wins on guarantees and worst case; ITS+Y wins best case and on
   lock-dominated kernels.** The paper should present this asymmetry, not hide it.
4. **Instruction / compiler cost** — TSPLIT: 1 operandless instruction, structured CFG preserved,
   ...
   **TSPLIT wins decisively.**
5. **Forward progress** — **no longer a TSPLIT exclusive.** Completing the ITS arm (ITS+Y) gives
...
6. **Program coverage** — TSPLIT and ITS+Y both run 10/10; baseline and ITS-CB (barriers only)
   run 7/10.

**Bottom line for the paper.** The strongest defensible thesis is not "TSPLIT alone provides
forward progress" — completing the ITS arm disproves that — but **"TSPLIT matches ITS's
...
`VX_CFG_DIVERGE_TYPE={DEFAULT|SPLIT|NV_ITS}` selects the architecture end-to-end; synthesis runs
