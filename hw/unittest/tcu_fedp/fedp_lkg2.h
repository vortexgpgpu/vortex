// fedp4_lkg2_lzc_fixed_v2.h
// FEDP with operand-LZC norm-adjust and adaptive reference exponent (no extra post-CPA C adder)
// FIX: final CPA is signed (int128), no unsigned clamp; single resetFlags removal.
// Enable tracing with -DFEDP_TRACE=1
#pragma once
#include <algorithm>
#include <array>
#include <cassert>
...
public:
                   ...
                   int sig_bits)
  {
    ...
    const auto [Sp32, Cp32, Emax_p, sticky_alignP] = align_and_reduce_products(groups);

    // Choose Eref to avoid unnecessary down-shift of C when gap fits Wc
    const int32_t Ec = c_term.E;
    const int32_t Eref = std::max(Ec, Emax_p - int32_t(Wc_-1u));

    // Retiming (Emax_p → Eref) in 64-bit sign-magnitude
    bool sticky_retime = false;
    int64_t Sp = Sp32, Cp = Cp32;
    retime_pair_to_Eref(Sp, Cp, Emax_p, Eref, sticky_retime);

    // Align C to Eref (RIGHT shift only; Eref >= Ec)
    bool sticky_c = false;
    const int32_t dC = Eref - Ec;
    const int64_t Cref = sign_mag_shr64(c_term.S, (uint32_t)dC, sticky_c);

    LOG("[retime] Emax_p=0x%x, Ec=0x%x, Eref=0x%x, dP=%d, dC=%d, Sp'=0x%llx, Cp'=0x%llx, Cref=0x%llx, sticky+=(%u|%u|%u)\n",
        (unsigned)Emax_p, (unsigned)Ec, (unsigned)Eref, int(Emax_p-Eref), dC,
        (unsigned long long)Sp, (unsigned long long)Cp, (unsigned long long)Cref,
        (unsigned)sticky_alignP, (unsigned)sticky_retime, (unsigned)sticky_c);

    // Final CSA
    uint64_t S=0, Cc=0;
    std::tie(S, Cc) = csa64((uint64_t)Sp, ((uint64_t)Cp)<<1, (uint64_t)Cref);
    LOG("[acc-csa-final] S=0x%llx, C=0x%llx\n",
        (unsigned long long)S, (unsigned long long)Cc);

    // ***** FIX: final CPA must be SIGNED *****
    __int128 V128s = ( __int128 )( (int64_t)S ) + ( ( (__int128)( (int64_t)Cc ) ) << 1 );
    // No clamp in normal ranges; but guard just in case:
    int64_t acc64 = (V128s > (__int128)INT64_MAX) ? (fflags_|=FLAG_NX, INT64_MAX)
                   : (V128s < (__int128)INT64_MIN) ? (fflags_|=FLAG_NX, INT64_MIN)
                   : (int64_t)V128s;

    LOG("[acc-cpa] acc64=0x%llx (signed=%lld)\n",
        (unsigned long long)acc64, (long long)acc64);

    const bool sticky_any = (sticky_mul || sticky_alignP || sticky_retime || sticky_c);
    if (acc64 == 0){
    ...
    const norm_t nrm = normalize64(acc64, Eref, sticky_any);
...
private:
  ...
  struct grp_t { int32_t S{0}, C{0}, E{0}; };
  ...
  static inline int32_t sign_mag_shr32(int32_t v, uint32_t k, bool &st){
    if (k==0||v==0) return v; uint32_t m = (v<0)? uint32_t(-v) : uint32_t(v);
    if (k>=32){ st |= (m!=0); return 0; } st |= ((m & ((1u<<k)-1u))!=0u);
    uint32_t m2 = m>>k; return (v<0)? -int32_t(m2) : int32_t(m2);
  }
  static inline int64_t sign_mag_shr64(int64_t v, uint32_t k, bool &st){
    if (k==0||v==0) return v; uint64_t m = (v<0)? uint64_t(-v) : uint64_t(v);
    if (k>=64){ st |= (m!=0); return 0; } st |= ((m & ((1ULL<<k)-1ULL))!=0ULL);
    uint64_t m2 = m>>k; return (v<0)? -int64_t(m2) : int64_t(m2);
  }
  static inline int64_t sign_mag_shl64(int64_t v, uint32_t k, bool &st){
    if (k==0||v==0) return v;
    __uint128_t m = (v<0)? __uint128_t(-( __int128)v) : __uint128_t(v);
    __uint128_t mL = m<<k; // No clamp expected for Wc<=24 and small d; keep a guard:
    if ((mL>>63) != 0){ st = true; return (v<0)? INT64_MIN+1 : INT64_MAX; }
    return (v<0)? -int64_t((uint64_t)mL) : (int64_t)(uint64_t)mL;
  }
  static inline int lzc_n(uint32_t x, uint32_t width){
    if (width==0) return 0; if (x==0) return int(width);
    if (width>=32) return clz32(x);
    uint32_t y = x << (32-width); return clz32(y);
  }
                ...
                int eb, int sb, bool packed)
  {
    ...
    for (uint32_t w=0; w<n_words; ++w){
      ...
      for (uint32_t i=0;i<epw;++i){
        ...
        out.push_back({a,b}); if (packed){ aw>>=width; bw>>=width; }
      }
    }
  ...
  CT decoded_to_common(const dec_t& d, int eb, int sb){
    ...
    const int32_t Ec = int32_t((d.exp? d.exp:0) - bias) + 127;
    const uint32_t M = ((d.exp!=0)? (1u<<sb):0u) | d.frac;
    const int dM = int(Wc_-1u) - sb;
    const uint32_t m = (dM>=0)? (M<<dM) : (M>>(-dM));
  ...
  uint32_t decode_special_or_zero(const std::vector<std::array<dec_t,2>>& ab_dec,
    ...
    for (size_t i=0;i<ab_dec.size();++i){
      ...
      if (a.is_inf||b.is_inf){ const uint32_t s=a.sign^b.sign; if (s) has_neg_inf=true; else has_pos_inf=true; LOG("[mul-prod] i=%zu, special=Inf/NaN/0*Inf\n", i); }
    }
                         ...
                         int eb, int sb)
  {
    ...
    const uint32_t Wm_in=uint32_t(sb)+1u, Wraw=2u*Wm_in;
    ...
    for (size_t i=0;i<terms.size();++i){
      ...
      const int lzc_a=lzc_n(Ma,Wm_in), lzc_b=lzc_n(Mb,Wm_in);
      const uint32_t plus1 = (a_norm && b_norm && ((a.frac+b.frac) >= (1u<<sb)))? 1u:0u;

      const int32_t Ea=int32_t(a.exp)-bias, Eb=int32_t(b.exp)-bias;
      const int32_t E = Ea + Eb + 127 + int32_t(plus1) - int32_t(lzc_a + lzc_b);
      const uint32_t top_est = 2u*sb - uint32_t(lzc_a + lzc_b) + plus1;

      const uint32_t P = Ma*Mb; // <=32b here
      int shift=int(Wc_-1) - int(top_est);
      uint32_t m_wc=0;
      if (shift>=0){ m_wc=(shift>=32)?0u:(P<<shift); }
      else { uint32_t r=uint32_t(-shift); if (r>=32){ sticky|=(P!=0); m_wc=0; }
  ...
  // --------- S2: groups → one CSA pair at Emax_p ---------
  ...
  // --------- S3: retime pair from Emax_p → Eref (64-bit) ---------
  void retime_pair_to_Eref(int64_t& Sp, int64_t& Cp, int32_t Emax_p, int32_t Eref, bool& st){
    const int32_t d = Emax_p - Eref; if (d==0) return;
    if (d>0){ Sp = sign_mag_shl64(Sp,(uint32_t)d,st); Cp = sign_mag_shl64(Cp,(uint32_t)d,st);
      LOG("[retimeP] LEFT  d=%d, Sp→0x%llx, Cp→0x%llx, sticky=%u\n", d,(unsigned long long)Sp,(unsigned long long)Cp,(unsigned)st);
    } else { const uint32_t r=(uint32_t)(-d);
      Sp = sign_mag_shr64(Sp,r,st); Cp = sign_mag_shr64(Cp,r,st);
      LOG("[retimeP] RIGHT d=%d, Sp→0x%llx, Cp→0x%llx, sticky=%u\n", d,(unsigned long long)Sp,(unsigned long long)Cp,(unsigned)st);
    }
  }

  // -------------------- S4: normalize/round/pack (64b) ---------------------
  norm_t normalize64(int64_t acc, int32_t Eref, bool sticky_prev){
    ...
    n.e_unb = (Eref - int32_t(Wc_-1u)) + (nbits - 1);

    uint64_t kept24=0, round_bit=0; bool sticky=sticky_prev;
    if (nbits>24){ const int sh=nbits-24; const uint64_t rem=(sh>=64)?mag:(mag & ((1ULL<<sh)-1ULL));
