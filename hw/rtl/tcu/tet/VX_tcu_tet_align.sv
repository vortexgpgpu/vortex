...
module VX_tcu_tet_align import VX_tcu_pkg::*; #(
    ...
    wire [TCU_EXP_BITS-1:0] max_exp_w;
    ...
    wire [TCU_EXP_BITS-1:0] s1_max_exp;
    wire [N-1:0][7:0]      s1_shift_amts;
    wire [N-1:0][WI-1:0]   s1_sigs_in;
    wire                   s1_is_int;
    wire                   s1_valid;
    wire [31:0]            s1_req_id;

    VX_tcu_tet_register #(
        .DATAW (TCU_EXP_BITS + (N * 8) + (N * WI) + 1 + 1 + 32),
        .DEPTH (1)
    ) pipe_align0 (
        .clk      (clk),
        .reset    (reset),
        .enable   (enable),
        .data_in  ({max_exp_w,  shift_amts,    sigs_in,    is_int,    valid_in,  req_id}),
        .data_out ({s1_max_exp, s1_shift_amts, s1_sigs_in, s1_is_int, s1_valid, s1_req_id})
    );
    ...
    for (genvar i = 0; i < N; ++i) begin : g_align_lanes
        wire [7:0] shift_amt = s1_shift_amts[i];

        wire in_sign = s1_sigs_in[i][WI-1];
        wire [WI-2:0] in_mag = s1_sigs_in[i][WI-2:0];
        ...
        assign sigs_out[i] = s1_is_int ? WO'($signed(s1_sigs_in[i])) : {in_sign, adj_mag};
    end
...
`ifdef DBG_TRACE_TCU
    always_ff @(posedge clk) begin
        if (s1_valid) begin
            `TRACE(4, ("%t: %s FEDP-ALIGN(%0d): is_int=%0d", $time, INSTANCE_ID, s1_req_id, s1_is_int));
            `TRACE(4, (", max_exp=0x%0h, shift_amts=", max_exp));
            `TRACE_ARRAY1D(4, "0x%0h", s1_shift_amts, N)
            `TRACE(4, (", sigs_in="));
            `TRACE_ARRAY1D(4, "0x%0h", s1_sigs_in, N)
            `TRACE(4, (", sigs_out="));
