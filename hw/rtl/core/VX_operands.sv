...
`endif
    ...
    localparam OUT_ARB_STICKY = (`VX_CFG_NUM_OPCS != 1) && (SIMD_COUNT != 1);

`ifdef PERF_ENABLE
    wire [`VX_CFG_NUM_OPCS-1:0][PERF_CTR_BITS-1:0] per_opc_perf_stalls;
`endif

    VX_operands_if per_opc_operands_if[`VX_CFG_NUM_OPCS]();

    wire [NUM_OPCS_W-1:0] sb_opc, wb_opc;
    if (`VX_CFG_NUM_OPCS != 1) begin : g_wis_opc
    ...
    wire [`VX_CFG_NUM_OPCS-1:0] scoreboard_ready_in;
    assign scoreboard_if.ready = scoreboard_ready_in[sb_opc];

    for (genvar i = 0; i < `VX_CFG_NUM_OPCS; i++) begin : g_collectors
        ...
        VX_opc_unit #(
            ...
            .NUM_BANKS    (`VX_CFG_NUM_GPR_BANKS),
            .OUT_BUF      (3)
        ) opc_unit (
            .clk          (clk),
            .reset        (reset),
