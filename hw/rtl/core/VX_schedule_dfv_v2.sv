...
`endif
    ...
    //==========================================================================
    // DFV Observability Primitives: Expose internal signals
    //==========================================================================

    // Observe active warps bitmap
    VX_dfv_bitmap_observe #(
        .NUM_BITS(`NUM_WARPS)
    ) obs_active_warps (
        .internal_bits(active_warps),
        .observed_bitmap(dfv_obs_active_warps)
    );

    // Observe stalled warps bitmap
    VX_dfv_bitmap_observe #(
        .NUM_BITS(`NUM_WARPS)
    ) obs_stalled_warps (
        .internal_bits(stalled_warps),
        .observed_bitmap(dfv_obs_stalled_warps)
    );

    // Observe ready warps (computed from active & ~stalled)
    wire [`NUM_WARPS-1:0] ready_warps_internal = active_warps & ~stalled_warps;
    VX_dfv_bitmap_observe #(
        .NUM_BITS(`NUM_WARPS)
    ) obs_ready_warps (
        .internal_bits(ready_warps_internal),
        .observed_bitmap(dfv_obs_ready_warps)
    );

    // Observe barrier masks
    VX_dfv_observe #(
        .WIDTH(`NUM_BARRIERS * `NUM_WARPS)
    ) obs_barrier_masks (
        .internal_signal(barrier_masks),
        .observed_signal(dfv_obs_barrier_masks)
    );

    // Observe barrier counters
    VX_dfv_observe #(
        .WIDTH(`NUM_BARRIERS * NW_WIDTH)
    ) obs_barrier_ctrs (
        .internal_signal(barrier_ctrs),
        .observed_signal(dfv_obs_barrier_ctrs)
    );

    // Count consecutive idle cycles (when scheduler has work but can't schedule)
    wire scheduler_idle_event = !schedule_valid_normal && (active_warps != 0);
    VX_dfv_consecutive_counter #(
        .WIDTH(16)
    ) idle_cycle_counter (
        .clk(clk),
        .reset(reset),
        .event(scheduler_idle_event),
        .count(dfv_obs_idle_cycles)
    );
...
`endif
    ...
    //==========================================================================
    // DFV Controllability Primitives: Override warp selection
    //==========================================================================

    // Force warp selection using dfv_force primitive
    VX_dfv_force #(
        .WIDTH(NW_WIDTH)
    ) force_warp_select (
        .dfv_enable(dfv_enable && dfv_force_warp_en),
        .dfv_value(dfv_force_warp_id),
        .normal_value(schedule_wid_normal),
        .output_value(schedule_wid)
    );

    // Force scheduler stall using dfv_force_stall primitive
    VX_dfv_force_stall force_sched_stall (
        .dfv_enable(dfv_enable),
        .dfv_force_stall(dfv_stall_scheduler),
        .normal_ready(schedule_valid_normal),
        .ready_out(schedule_valid)
    );
...
`endif
   ...
   // timeout handling
    ...
    VX_dfv_observe #(
        ...
        .internal_signal(timeout_ctr[15:0]),
        .observed_signal(dfv_obs_timeout_ctr)
    );
