...
`endif
    ...
    // Insert demo stage between request crossbar and banks ////////////////////
`ifdef VM_ENABLE
    wire [NUM_BANKS-1:0]                        per_bank_core_req_valid_d;
    wire [NUM_BANKS-1:0][`CS_LINE_ADDR_WIDTH-1:0] per_bank_core_req_addr_d;
    wire [NUM_BANKS-1:0]                        per_bank_core_req_rw_d;
    wire [NUM_BANKS-1:0][WORD_SEL_WIDTH-1:0]    per_bank_core_req_wsel_d;
    wire [NUM_BANKS-1:0][WORD_SIZE-1:0]         per_bank_core_req_byteen_d;
    wire [NUM_BANKS-1:0][`CS_WORD_WIDTH-1:0]    per_bank_core_req_data_d;
    wire [NUM_BANKS-1:0][TAG_WIDTH-1:0]         per_bank_core_req_tag_d;
    wire [NUM_BANKS-1:0][REQ_SEL_WIDTH-1:0]     per_bank_core_req_idx_d;
    wire [NUM_BANKS-1:0][`UP(FLAGS_WIDTH)-1:0]  per_bank_core_req_flags_d;
    wire [NUM_BANKS-1:0]                        per_bank_core_req_ready_d;
`endif

    // Banks access ///////////////////////////////////////////////////////////

    // Per-bank demo stage between crossbar and banks
`ifdef VM_ENABLE
    for (genvar bank_id = 0; bank_id < NUM_BANKS; ++bank_id) begin : g_demo_stage
        demo_module #(
            .ADDR_WIDTH   (`CS_LINE_ADDR_WIDTH),
            .WSEL_WIDTH   (WORD_SEL_WIDTH),
            .BYTEEN_WIDTH (WORD_SIZE),
            .DATA_WIDTH   (`CS_WORD_WIDTH),
            .TAG_WIDTH    (TAG_WIDTH),
            .IDX_WIDTH    (REQ_SEL_WIDTH),
            .FLAGS_WIDTH  (`UP(FLAGS_WIDTH)),
            .MEM_PORTS    (MEM_PORTS),  //addon
            .MEM_ARB_SEL_WIDTH (MEM_ARB_SEL_WIDTH), //addon
            .PROCESS_ADDR (0)
        ) demo_stage (
            .clk       (clk),
            .reset     (reset),

            .in_valid  (per_bank_core_req_valid[bank_id]),
            .in_addr   (per_bank_core_req_addr[bank_id]),
            .in_rw     (per_bank_core_req_rw[bank_id]),
            .in_wsel   (per_bank_core_req_wsel[bank_id]),
            .in_byteen (per_bank_core_req_byteen[bank_id]),
            .in_data   (per_bank_core_req_data[bank_id]),
            .in_tag    (per_bank_core_req_tag[bank_id]),
            .in_idx    (per_bank_core_req_idx[bank_id]),
            .in_flags  (per_bank_core_req_flags[bank_id]),
            .in_ready  (per_bank_core_req_ready[bank_id]),

            .out_valid (per_bank_core_req_valid_d[bank_id]),
            .out_addr  (per_bank_core_req_addr_d[bank_id]),
            .out_rw    (per_bank_core_req_rw_d[bank_id]),
            .out_wsel  (per_bank_core_req_wsel_d[bank_id]),
            .out_byteen(per_bank_core_req_byteen_d[bank_id]),
            .out_data  (per_bank_core_req_data_d[bank_id]),
            .out_tag   (per_bank_core_req_tag_d[bank_id]),
            .out_idx   (per_bank_core_req_idx_d[bank_id]),
            .out_flags (per_bank_core_req_flags_d[bank_id]),
            .out_ready (per_bank_core_req_ready_d[bank_id]),
//addon start
            // Tap memory arbiter outputs for visibility/control
            .arb_mem_req_valid   (mem_req_valid),
            .arb_mem_req_ready   (mem_req_ready),
            .arb_mem_req_sel_out (mem_req_sel_out),

            // Drive additional demo requests into mem arbiter inputs
            .mem_req_valid (demo_mem_req_valid[bank_id]),
            .mem_req_addr  (demo_mem_req_addr[bank_id]),
            .mem_req_rw    (demo_mem_req_rw[bank_id]),
            .mem_req_byteen(demo_mem_req_byteen[bank_id]),
            .mem_req_data  (demo_mem_req_data[bank_id]),
            .mem_req_tag   (demo_mem_req_tag[bank_id]),
            .mem_req_flags (demo_mem_req_flags[bank_id]),
            .mem_req_ready (demo_mem_req_ready[bank_id])
//addon end
        );
    end
`endif
    ...
    for (genvar bank_id = 0; bank_id < NUM_BANKS; ++bank_id) begin : g_banks
        ...
        `endif
            ...
            // Core request
        `ifdef VM_ENABLE
            .core_req_valid     (per_bank_core_req_valid_d[bank_id]),
            .core_req_addr      (per_bank_core_req_addr_d[bank_id]),
            .core_req_rw        (per_bank_core_req_rw_d[bank_id]),
            .core_req_wsel      (per_bank_core_req_wsel_d[bank_id]),
            .core_req_byteen    (per_bank_core_req_byteen_d[bank_id]),
            .core_req_data      (per_bank_core_req_data_d[bank_id]),
            .core_req_tag       (per_bank_core_req_tag_d[bank_id]),
            .core_req_idx       (per_bank_core_req_idx_d[bank_id]),
            .core_req_flags     (per_bank_core_req_flags_d[bank_id]),
            .core_req_ready     (per_bank_core_req_ready_d[bank_id]),
        `else
...
`ifdef VM_ENABLE
    // Extra per-bank demo requests into mem arbiter ///////////////////////////
    wire [NUM_BANKS-1:0]                        demo_mem_req_valid;
    wire [NUM_BANKS-1:0][`CS_LINE_ADDR_WIDTH-1:0] demo_mem_req_addr;
    wire [NUM_BANKS-1:0]                        demo_mem_req_rw;
    wire [NUM_BANKS-1:0][LINE_SIZE-1:0]         demo_mem_req_byteen;
    wire [NUM_BANKS-1:0][`CS_LINE_WIDTH-1:0]    demo_mem_req_data;
    wire [NUM_BANKS-1:0][BANK_MEM_TAG_WIDTH-1:0] demo_mem_req_tag;
    wire [NUM_BANKS-1:0][`UP(FLAGS_WIDTH)-1:0]  demo_mem_req_flags;
    wire [NUM_BANKS-1:0]                        demo_mem_req_ready;
`endif
    VX_stream_arb #(
    `ifdef VM_ENABLE
        .NUM_INPUTS (NUM_BANKS*2),
    `else
        .NUM_INPUTS (NUM_BANKS),
    `endif
        ...
        .reset     (reset),
    `ifdef VM_ENABLE
        .valid_in  ({demo_mem_req_valid, per_bank_mem_req_valid}),
        .data_in   ({
            {demo_mem_req_rw, demo_mem_req_addr, demo_mem_req_data, demo_mem_req_byteen, demo_mem_req_flags, demo_mem_req_tag},
            per_bank_mem_req_pdata
        }),
        .ready_in  ({demo_mem_req_ready, per_bank_mem_req_ready}),
    `else
