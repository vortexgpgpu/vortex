...
// Banked L1 TLB storage + miss station, an alternative to VX_tlb_l1 behind
// the same parent (VX_mmu) contract. The entry array is split into
// NUM_BANKS single-ported banks selected by the low VPN bits: each bank
// answers at most one lane per cycle (bank_conflict tells the parent to
// hold the other lanes), and each bank holds at most one parked miss —
// a miss blocks its bank for the walk's duration while the other banks
// keep hitting. A same-VPN request waits on its busk's in-flight walk
// (mshr_match) instead of joining a queue. Trades the baseline's
// full multi-port CAM + MSHR for per-bank ports and slots: cheaper
// lookup hardware at scale, one outstanding walk per bank.
module VX_tlb_l1_banked import VX_gpu_pkg::*, VX_tlb_pkg::*; #(
