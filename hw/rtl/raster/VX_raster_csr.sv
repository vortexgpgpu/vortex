...
module VX_raster_csr import VX_gpu_pkg::*, VX_raster_pkg::*; #(
    ...
    raster_csrs_t [`VX_CFG_NUM_THREADS-1:0] wdata;
    raster_csrs_t [`VX_CFG_NUM_THREADS-1:0] rdata;
    reg [`VX_CFG_NUM_THREADS-1:0]           write;
    reg [NW_WIDTH-1:0]              waddr;
    wire [NW_WIDTH-1:0]             raddr;

    // Per-(wid, slot) storage: NUM_THREADS slots per warp (one per lane × pid).
    for (genvar i = 0; i < `VX_CFG_NUM_THREADS; ++i) begin : g_stamp_store
        VX_dp_ram #(
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin : g_read_data
        ...
        reg [`VX_CFG_XLEN-1:0] selected;
        always @(*) begin
            case (raster_csr_if.read_addr)
                `VX_CSR_RASTER_POS_MASK:  selected = `VX_CFG_XLEN'(lane_csrs.pos_mask);
                `VX_CSR_RASTER_PID:       selected = `VX_CFG_XLEN'(lane_csrs.pid);
                `VX_CSR_RASTER_BCOORD_X0: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[0][0]);
                `VX_CSR_RASTER_BCOORD_X1: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[0][1]);
                `VX_CSR_RASTER_BCOORD_X2: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[0][2]);
                `VX_CSR_RASTER_BCOORD_X3: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[0][3]);
                `VX_CSR_RASTER_BCOORD_Y0: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[1][0]);
                `VX_CSR_RASTER_BCOORD_Y1: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[1][1]);
                `VX_CSR_RASTER_BCOORD_Y2: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[1][2]);
                `VX_CSR_RASTER_BCOORD_Y3: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[1][3]);
                `VX_CSR_RASTER_BCOORD_Z0: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[2][0]);
                `VX_CSR_RASTER_BCOORD_Z1: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[2][1]);
                `VX_CSR_RASTER_BCOORD_Z2: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[2][2]);
                `VX_CSR_RASTER_BCOORD_Z3: selected = `VX_CFG_XLEN'(lane_csrs.bcoords[2][3]);
                default:                  selected = '0;
            endcase
        end
