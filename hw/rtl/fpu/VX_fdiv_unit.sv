...
`endif
    ...
    `STATIC_ASSERT(!USE_VENDOR_IP || (LATENCY == 28),
        ("vendor xil_fdiv latency is 28; set VX_CFG_FDIV_LATENCY=28"))
    ...
    `ifdef QUARTUS
        ...
        assign fflags = '0; // acl_fdiv does not expose exception flags
    `else // VIVADO
        wire [3:0] tuser;
        xil_fdiv fdiv (
            .aclk                (clk),
        ...
        // tuser = {div0, invalid, overflow, underflow} -> fflags {NV, DZ, OF, UF, NX}.
        assign fflags = EXCEPT_ENABLE ? {tuser[2], tuser[3], tuser[1], tuser[0], 1'b0} : '0;
    `endif
