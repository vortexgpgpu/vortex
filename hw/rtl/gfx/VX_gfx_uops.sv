...
// right file.
//
// It serves BOTH graphics macro-ops, because a sequencer slot is expensive: each
// one is a full ibuffer_t input on the sequencer's output mux (uop_out_data
// [UOP_MAX]) plus a bit of its priority encoder. The two are combinational
// rewrites of the same ibuf_in and their selects are mutually exclusive --
// EX_SFU + INST_SFU_GFXW for the window ops, EX_LSU + a non-zero export_mask for
// the fragment export -- so they share one slot instead of owning two.
//
...
//
module VX_gfx_uops import VX_gfx_window_pkg::*, VX_gpu_pkg::*; (
    input clk,
    input reset,
...
`endif
        if (is_trace) begin
            ...
            2'd1: begin // ORIGIN: f0,f1,f2 -> origin slots
                ...
                ibuf_r.op_args.gfxw.slot = 5'(`VX_RT_RAY_ORIGIN);
            ...
            2'd2: begin // DIR: f3,f4,f5 -> direction slots
                ...
                ibuf_r.op_args.gfxw.slot = 5'(`VX_RT_RAY_DIRECTION);
            ...
            default: begin // ARM (uop 3): f6,f7 -> tmin/tmax, then arm the walk
                ...
                ibuf_r.op_args.gfxw.slot = 5'(`VX_RT_T_MIN);
