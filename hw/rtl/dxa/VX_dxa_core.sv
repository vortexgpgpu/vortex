...
`ifdef VX_CFG_EXT_DXA_S2G_ENABLE
    `STATIC_ASSERT(DXA_LMEM_OUT_TAG_W == (DXA_LMEM_TAG_W
        + `ARB_SEL_BITS(`VX_CFG_NUM_DXA_CORES, 1)),
        ("DXA worker arb tag width does not preserve the response route"))
`endif
...
`ifdef VX_CFG_EXT_DXA_S2G_ENABLE
    `STATIC_ASSERT($bits(worker_smem_bus_if[0].req_data.tag) == DXA_LMEM_TAG_W,
        ("DXA worker request tag width mismatch"))
    `STATIC_ASSERT($bits(worker_smem_bus_if[0].rsp_data.tag) == DXA_LMEM_TAG_W,
        ("DXA worker response tag width mismatch"))
    `STATIC_ASSERT($bits(smem_bus_if[0].req_data.tag) == DXA_LMEM_OUT_TAG_W,
        ("DXA worker-arb output request tag width mismatch"))
    `STATIC_ASSERT($bits(smem_bus_if[0].rsp_data.tag) == DXA_LMEM_OUT_TAG_W,
        ("DXA worker-arb output response tag width mismatch"))
`endif
