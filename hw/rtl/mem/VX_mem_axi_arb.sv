...
// ============================================================================
// VX_mem_axi_arb — N-master to M-slave AXI4 arbiter on VX_mem_axi_if bundles.
//
// Interface convenience wrapper over the flat-port libs core VX_mm_axi_arb.
// MULTI_OUT selects the arbitration contract:
//   0 : single-outstanding (ownership) — masters serialize per channel.
//   1 : multi-outstanding (ID-routed)  — NUM_OUTPUTS must be 1; the top
//       LOG2UP(NUM_INPUTS) bits of the AXI ID carry the source index, so
//       reads stay concurrent and write responses (B) stay concurrent.
//
// The reduced core carries no size/burst sideband: inputs are sunk, outputs
// driven to full-width INCR (the only shape the CP/AFU masters use).
// ============================================================================

module VX_mem_axi_arb #(
    ...
    localparam AXSIZE = `CLOG2(STRB_WIDTH);   // log2 bytes/beat (full-width)
