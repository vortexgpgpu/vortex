...
`ifdef VX_CFG_EXT_DXA_S2G_ENABLE
    `STATIC_ASSERT(!DMA_ENABLE || ($bits(dma_bus_if.req_data.tag) == DMA_TAG_WIDTH),
        ("local-memory DMA request tag width mismatch"))
    `STATIC_ASSERT(!DMA_ENABLE || ($bits(dma_bus_if.rsp_data.tag) == DMA_TAG_WIDTH),
        ("local-memory DMA response tag width mismatch"))
`endif
