...
# On-chip clock
create_clock -name clk -period 0.4 [get_ports clk]  ;# 1.5 GHz
set_clock_uncertainty 0.04 [get_clocks clk]

# Virtual external clock for I/O timing
create_clock -name ext_clk -period 0.4

# I/O delays: exclude the clock port from input delays
set in_ports [filter_collection [all_inputs] "name != clk"]
set_input_delay  0.05 -clock ext_clk $in_ports
set_output_delay 0.05 -clock ext_clk [all_outputs]
