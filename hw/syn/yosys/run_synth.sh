...
SAIF_FILE="${SAIF_FILE:-}"
SAIF_INST="${SAIF_INST:-}"
SAIF_ACTIVITY_TCL="${SAIF_ACTIVITY_TCL:-}"
...
if [[ "$RUN_STA" == "1" ]]; then
  ...
  STA_SDC="$RESOLVED_SDC"; [[ -n "$STA_SDC" ]] || STA_SDC="$SDC_FILE"
  ...
  TOP=$TOP NETLIST="$NETLIST" LIB_TGT="$LIB_TGT" LIB_ROOT="$LIB_ROOT" SDC_FILE="$STA_SDC" RPT_DIR="$RPT_DIR" SAIF_FILE="$SAIF_FILE" SAIF_INST="$SAIF_INST" SAIF_ACTIVITY_TCL="$SAIF_ACTIVITY_TCL" sta "$STA_SCRIPT" > "$RPT_DIR/sta.log" 2>&1
  cat "$RPT_DIR/sta.log"
  stamp "sta"
fi
