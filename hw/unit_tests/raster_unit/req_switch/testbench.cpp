...
void eval(VVX_raster_req_switch* dut, VerilatedVcdC* m_trace)
{
    dut->clk = not dut->clk;
    dut->eval();
    timestamp += 10;
...
int main(int argc, char** argv, char** env) {
    VVX_raster_req_switch *dut = new VVX_raster_req_switch();
    ...
    dut->x_loc = 0; dut->y_loc = 256;
    dut->edge_func_val[0] = 518; dut->edge_func_val[1] = 42976; dut->edge_func_val[2] = 0;
    dut->mem_base_addr = 10; dut->mem_stride = 1;
    eval(dut, m_trace);
    dut->input_valid = 1;
    // Return data from me streamer
    dut->mem_rsp_valid = 1;
    dut->raster_mem_rsp_tag = 0;
    //dut->mem_rsp_data = 100;
    dut->raster_slice_ready = 1;
    eval(dut, m_trace);
    dut->input_valid = 1;
    // Return data from me streamer
    dut->mem_rsp_valid = 1;
    dut->raster_mem_rsp_tag = 1;
    //dut->mem_rsp_data = 100;
    dut->raster_slice_ready = 4;
