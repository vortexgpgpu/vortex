...
int main(int argc, char *argv[]) {
  ...
  kernel_arg.logw          = src_logwidth;
  kernel_arg.logh          = src_logheight;
  ...
  // ---- self-check: every output word must be 0 -------------------------
  // The kernel writes XOR(vx_tex4_quad texel, vx_tex at the same HW LOD) per
  // pixel; a non-zero word means the quad/HW-LOD path disagreed with vx_tex.
  ...
  const uint32_t* words = reinterpret_cast<const uint32_t*>(dst_pixels.data());
  for (uint32_t i = 0; i < dst_bufsize / 4; ++i) {
    if (words[i] != 0) ++mismatches;
  }
  if (0 == mismatches) {
    std::cout << "PASSED!" << std::endl;
    return 0;
  }
  std::cout << "FAILED: " << mismatches << " of " << (dst_bufsize / 4)
            << " pixels: vx_tex4 quad != vx_tex at the hardware LOD" << std::endl;
  return mismatches;
}
