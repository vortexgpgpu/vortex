...
# Non-native values (aarch64, armhf) produce a suffixed binary linked
# against a cross-compiled runtime stub. The .vxbin always builds with
# the RISC-V toolchain regardless.
#
# Cross-compiled ELFs use the default dynamic linker (PT_INTERP) so that
# gem5's setInterpDir() can prepend a sysroot to the interpreter lookup.
HOST_ARCH ?= x86_64
...
# Binary name, suffixed by HOST_ARCH for non-native builds.
APP := $(PROJECT)$(PROJECT_SUFFIX)

all: $(APP) kernel.vxbin kernel.dump

# Force rebuild when CONFIGS change; PID-suffixed tmp tolerates concurrent invocations.
$(CONFIG_STAMP): FORCE
	@TMP=$@.tmp.$$$$ ; \
...
# Build the cross-compiled runtime stub for non-native HOST_ARCH.
