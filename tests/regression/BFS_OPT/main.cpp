...
void cleanup() {
  ...
  if (device) {
    ...
    vx_mem_free(device, kernel_arg.count_addr);
    vx_mem_free(device, kernel_arg.count1_addr);
...
//1. Allocate and initialize host memory
  ...
  uint32_t edgelist_bufsz = sizeof(int)*edge_list_size;
...
/* {
      ...
      buf_ptr_upload[0] = count;
  }
   RT_CHECK(vx_copy_to_dev(common_buf, kernel_arg.count_addr, hover_bufsz, 0));*/
...
//I. 7. upload kernel 1 arguments 
   ...
   {
      auto buf_ptr_upload = (int*)vx_host_ptr(common_buf);
      buf_ptr_upload[0] = hcount1;
   }
   RT_CHECK(vx_copy_to_dev(common_buf, kernel_arg.count1_addr, hover_bufsz, 0)); 

  {
      auto buf_ptr_upload = (int*)vx_host_ptr(common_buf);
      buf_ptr_upload[0] = hcount;
  }
   RT_CHECK(vx_copy_to_dev(common_buf, kernel_arg.count_addr, hover_bufsz, 0)); 
...
//I. 13. Copy results to host arrays
   ...
   /*RT_CHECK(vx_copy_from_dev(common_buf, kernel_arg.count1_addr, hover_bufsz, 0));
   {
    auto buf_ptr = (int32_t*)vx_host_ptr(common_buf);     
    count1 = buf_ptr[0]; 
    std::cout << "COUNT 1 IS " <<count1<<std::endl;
    }*/
    RT_CHECK(vx_copy_from_dev(common_buf, kernel_arg.count_addr, hover_bufsz, 0));
   {
    auto buf_ptr = (int32_t*)vx_host_ptr(common_buf);     
    hcount = buf_ptr[0]; 
    std::cout << "COUNT IS " <<hcount<<std::endl;
    }
hcount1 = hcount;
std::cout << "COUNT 1 IS " <<hcount1<<std::endl;    
