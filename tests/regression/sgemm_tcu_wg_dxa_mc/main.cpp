...
int main(int argc, char *argv[]) {
  ...
  // Inter-core multicast group spans VX_CFG_NUM_CORES (one CTA per core).
  uint64_t num_cores_cap = 0;
  RT_CHECK(vx_device_query(device, VX_CAPS_NUM_CORES, &num_cores_cap));
  const uint32_t mc_group_size = (uint32_t)num_cores_cap;
  if (mc_group_size < 2) {
    std::cout << "Error: sgemm_tcu_wg_dxa_mc requires VX_CFG_NUM_CORES >= 2 "
                 "(have " << num_cores_cap << "); use _mw on single-core\n";
    cleanup();
    return -1;
  }
  kernel_arg.mc_group_size = mc_group_size;
  std::cout << "mc_group_size=" << mc_group_size << " (inter-core multicast)\n";
  ...
  // Inter-core multicast: each receiver core's __local_mem() has the same
  // layout, so the per-receiver SMEM offset is identical. The socket-level
  // cross-core LMEM bus routes by core_id. smem_stride = 0 (no per-receiver
  // offset within the same logical SMEM address).
  RT_CHECK(vx_dxa_program_desc_multicast(device, kDescB, /*smem_stride=*/0));
