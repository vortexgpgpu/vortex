...
int main(int argc, char *argv[]) {
  ...
  // A [cta_M × tileK] + B [tileK × xtileN] (input_t) + C [num_warps × xtileM × xtileN] (output_t)
  uint32_t smem_size = (cta_M * wg_cfg::tileK + wg_cfg::tileK * per_warp_N) * sizeof(itype_t)
                     + (warps * per_warp_M * per_warp_N) * sizeof(otype_t);
  ...
  // WGMMA performance statistics
  {
    auto pct = [](uint64_t num, uint64_t den) -> double {
      return den ? 100.0 * num / den : 0.0;
    };

    uint64_t stall_tcu = 0, stall_tcu_opds = 0;
    vx_mpm_query(device, VX_DCR_MPM_CLASS_CORE, VX_CSR_MPM_STALL_TCU,          0, &stall_tcu);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_CORE, VX_CSR_MPM_STALL_TCU_OPDS,     0, &stall_tcu_opds);

    uint64_t wgmma_instrs = 0, wgmma_stalls = 0;
    uint64_t st_tbuf = 0, st_mdata = 0, st_pipe = 0;
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_WGMMA_INSTRS,   0, &wgmma_instrs);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_WGMMA_STALLS,   0, &wgmma_stalls);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_WGMMA_ST_TBUF,  0, &st_tbuf);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_WGMMA_ST_MDATA, 0, &st_mdata);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_WGMMA_ST_PIPE,  0, &st_pipe);

    uint64_t tbuf_fetches = 0, tbuf_fetch_st = 0;
    uint64_t tbuf_fetch_cyc = 0, fetch_b_cyc = 0;
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_TBUF_FETCHES,   0, &tbuf_fetches);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_TBUF_STALLS,    0, &tbuf_fetch_st);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_TBUF_FETCH_CYC, 0, &tbuf_fetch_cyc);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_FETCH_B_CYC,    0, &fetch_b_cyc);

    uint64_t lmem_reads = 0, lmem_reads_a = 0, lmem_reads_b = 0;
    uint64_t lmem_rsp_st = 0, lmem_arb_st = 0, lmem_bank_st = 0, compute_cyc = 0;
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_LMEM_READS,     0, &lmem_reads);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_LMEM_READS_A,   0, &lmem_reads_a);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_LMEM_READS_B,   0, &lmem_reads_b);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_LMEM_RSP_ST,    0, &lmem_rsp_st);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_LMEM_ARB_ST,    0, &lmem_arb_st);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_TCU,  VX_CSR_MPM_TCU_COMPUTE_CYC,    0, &compute_cyc);
    vx_mpm_query(device, VX_DCR_MPM_CLASS_MEM,  VX_CSR_MPM_LMEM_BANK_ST,       0, &lmem_bank_st);

    printf("=== WGMMA statistics ===\n");

    printf("-- dispatch --\n");
    printf("  stall_tcu:          %lu  (dispatch stalled, TCU pipeline full)\n",    stall_tcu);
    printf("  stall_tcu_opds:     %lu  (operand fetch stalled, reg bank conflict)\n", stall_tcu_opds);

    printf("-- issue pipeline --\n");
    uint64_t issue_total = wgmma_instrs + wgmma_stalls;
    printf("  wgmma_instrs:       %lu\n",                                             wgmma_instrs);
    printf("  wgmma_stalls:       %lu  (%.1f%% of issue slots)\n",                   wgmma_stalls,  pct(wgmma_stalls, issue_total));
    printf("  issue_efficiency:   %.1f%%\n",                                          pct(wgmma_instrs, issue_total));
    printf("  st_tbuf:            %lu  (%.1f%% of stalls — tbuf not ready)\n",       st_tbuf,  pct(st_tbuf,  wgmma_stalls));
    printf("  st_mdata:           %lu  (%.1f%% of stalls — mdata queue full)\n",     st_mdata, pct(st_mdata, wgmma_stalls));
    printf("  st_pipe:            %lu  (%.1f%% of stalls — FEDP not ready)\n",       st_pipe,  pct(st_pipe,  wgmma_stalls));

    printf("-- tile buffer --\n");
    printf("  tbuf_fetches:       %lu  (tile-miss alloc events)\n",                   tbuf_fetches);
    printf("  tbuf_fetch_st:      %lu  (wgmma stall cycles waiting for fetch)\n",    tbuf_fetch_st);
    printf("  tbuf_fetch_cyc:     %lu  (cycles FSM active)\n",                        tbuf_fetch_cyc);
    printf("  fetch_b_cyc:        %lu  (%.1f%% of fetch cycles in FETCH_B)\n",       fetch_b_cyc, pct(fetch_b_cyc, tbuf_fetch_cyc));

    printf("-- LMEM reads --\n");
    printf("  lmem_reads:         %lu\n",                                             lmem_reads);
    printf("  lmem_reads_a:       %lu  (%.1f%%)\n",                                  lmem_reads_a,   pct(lmem_reads_a, lmem_reads));
    printf("  lmem_reads_b:       %lu  (%.1f%%)\n",                                  lmem_reads_b,   pct(lmem_reads_b, lmem_reads));
    printf("  lmem_rsp_stalls:    %lu  (%.1f%% of fetch cycles stalled on rsp)\n",   lmem_rsp_st,    pct(lmem_rsp_st,  tbuf_fetch_cyc));
    printf("  lmem_arb_stalls:    %lu  (multi-block LMEM port contention)\n",        lmem_arb_st);
    printf("  lmem_bank_stalls:   %lu  (SMEM bank conflicts)\n",                     lmem_bank_st);

    printf("-- compute --\n");
    printf("  compute_cyc:        %lu  (FEDP pipeline active)\n",                    compute_cyc);
    printf("  compute/fetch_overlap: fetch_cyc=%lu  compute_cyc=%lu  ratio=%.2f\n",
           tbuf_fetch_cyc, compute_cyc,
           tbuf_fetch_cyc ? (double)compute_cyc / tbuf_fetch_cyc : 0.0);
  }
