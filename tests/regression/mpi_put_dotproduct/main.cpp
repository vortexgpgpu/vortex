...
int main(int argc, char* argv[]) {
  ...
  std::vector<TYPE> window_buf;
  if (rank == 0)
    window_buf.resize(world_size);

  MPI_Win win;
  MPI_Win_create(rank == 0 ? window_buf.data() : nullptr,
                 rank == 0 ? world_size * sizeof(TYPE) : 0,
                 sizeof(TYPE),
                 MPI_INFO_NULL,
                 MPI_COMM_WORLD,
                 &win);
  ...
  // Each rank writes its local_sum into slot [rank] on rank 0
  MPI_Put(&local_sum,
          1,
          MPI_FLOAT,
          0,
