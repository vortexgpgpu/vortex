// GPU-side program for mode 13 -- workgroup WGMMA, cooperative SW load (the DXA control for mode 12).
//
...
__kernel void moti_tcu_wg(kernel_arg_t* __UNIFORM__ arg) {
  ...
  for (uint32_t k = 0; k < K; k += kStK) {
    ...
    // produced. This is mode 12 with exactly one thing removed, so the pair measures
