...
// Standalone elementwise epilogue pass over D, used by DTCU modes 7/8.
//
...
// fuse the same math before the output store (see wmma_common.h), while the
...
//
// One block per row, one warp per block. The warp issues in lockstep, and vx_fence()
// orders each Local Memory store before the following lane-to-lane reduction load.
// Local Memory holds the blockDim-element reduction scratch.
//
...
//
// One warp owns NUM_THREADS adjacent columns and each lane owns one complete column.
// Thus a loop iteration touches one contiguous row, while each lane's accumulator still
// reduces down M.  The former one-warp-per-column version made all 32 lanes stride by N
// and then reduced them through Local Memory.  It computed the same value, but turned a
// 256x128 column pass into a 30+ minute SimX job and made the documented sweep
// impractical.  This is also the conventional GPU transpose of a column reduction: map
// lanes across columns so global-memory transactions are coalesced.
