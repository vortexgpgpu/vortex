...
// dtensor_desc_t and DTENSOR_FLAG_* come from <dtcu_cfg.h> (pulled in by tensor_cfg.h);
// they used to be mirrored here, which meant a silent ABI drift whenever the real
// struct changed.
...
static int run_case(uint32_t mode,
  ...
  if (mode == 0) {
    ...
    // WMMA (naive 1/2 or pipelined 5/6): one block (one warp) per output tile.
    li.ndim = 2;
    ...
    else if (mode == 6) li.lmem_size = 2 * stage_bytes;  // double-buffer DXA
  } else {
...
static uint32_t g_app = 1;  // 1=baseline .. 8=dequant+softmax (Phase B; only 1 impl'd so far)
...
static void parse_args(int argc, char** argv) {
  ...
  while ((c = getopt(argc, argv, "s:a:h")) != -1) {
    ...
    case 'a': g_app = (uint32_t)atoi(optarg); break;
    case 'h':
      std::cout << "Usage: [-s size_mult] [-a app_id] (GEMM = size_mult * DTCU native tile)\n";
      exit(0);
    default: exit(-1);
    }
  }
...
int main(int argc, char** argv) {
  ...
  const char* names[7] = { "in-core SIMT", "in-core TCU", "in-core TCU + DXA", "DTCU (no TMA)", "DTCU + DTCU_TMA", "TCU pipelined", "TCU+DXA pipelined" };
  ...
  for (uint32_t m = 0; m < 7; ++m) {
    ...
    std::cout << "[MOTI] app=" << g_app << " size=" << g_size_mult
              << " M=" << M << " N=" << N << " K=" << K
  ...
  int total_errors = 0;
  for (uint32_t m = 0; m < 7; ++m) total_errors += mode_errors[m];
