...
static void show_usage() {
  ...
  std::cout << "  -f:       issue DXA then return without waiting (lifetime/counter test)\n";
}
...
#ifndef DXA_LIFETIME_FIRE_FORGET
  if (fire_and_forget) {
    std::cerr << "Error: -f requires CONFIGS+=-DDXA_LIFETIME_FIRE_FORGET\n";
    exit(1);
  }
#endif
}
...
int main(int argc, char* argv[]) {
  ...
  kernel_arg.fire_and_forget = fire_and_forget;
#endif
  ...
  std::vector<TYPE> h_dst(total_elems, static_cast<TYPE>(0));
  ...
  if (!fire_and_forget) {
    for (uint32_t i = 0; i < total_elems; ++i) {
      if (h_dst[i] != h_src[i]) {
        if (errors < 20) {
          std::cout << "mismatch at " << i << ": expected " << h_src[i]
                    << ", got " << h_dst[i] << "\n";
        }
        ++errors;
      }
  ...
  if (errors != 0) {
    std::cout << "FAILED (" << errors << " data mismatches)\n";
    cleanup();
    return -1;
  }
