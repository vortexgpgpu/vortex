...
// publishes a per-ray payload buffer pointer through
// VX_RT_PAYLOAD_PTR_LO. RtuCore commits the hit, then queues a
// CHS yield through the existing reformation/CB_YIELD path. The
// dispatcher reads hit_t + payload pointer, writes MAGIC ^ hit_t_bits
// to the payload, and exits via vx_rt_cb_ret(VX_RT_CB_DONE) + mret.
// Oracle: status=HIT, payload[0] = MAGIC ^ bits(5.0f).
...
int main(int /*argc*/, char* /*argv*/[]) {
  ...
  // Scene: 1 opaque triangle at z=5.
  ...
  *tri_flags = RTU_TRI_FLAG_OPAQUE;
  ...
  // Payload buffer: 4 bytes. Pre-init with a sentinel so we can tell
  // "CHS never ran" from "CHS ran and wrote 0".
  ...
  std::cout << "scene_addr=0x" << std::hex << kernel_arg.scene_addr << std::dec
            << ", payload_addr=0x" << std::hex << kernel_arg.payload_addr
            << std::dec << std::endl;
  ...
  // Oracle: HIT at t=5.0f. CHS wrote MAGIC ^ bits(5.0f) into payload.
  union { float f; uint32_t u; } exp_ht;
  exp_ht.f = 5.f;
  uint32_t exp_payload = RTU_CHS_MAGIC ^ exp_ht.u;
  uint32_t exp_status  = VX_RT_STS_DONE_HIT;
  std::cout << "oracle: status=" << exp_status
            << " hit_t=5 chs_payload=0x" << std::hex << exp_payload
            << std::dec << std::endl;

  int errors = 0;
  bool sts_ok = (result.status == exp_status);
  bool t_ok   = std::fabs(result.hit_t - 5.f) < 1e-4f;
  bool ch_ok  = (result.chs_payload == exp_payload);
  if (!sts_ok || !t_ok || !ch_ok) {
    std::cout << "result: status=" << result.status
              << " hit_t=" << result.hit_t
              << " chs_payload=0x" << std::hex << result.chs_payload
              << std::dec << std::endl;
    if (!ch_ok && result.chs_payload == 0xdeadbeef) {
      std::cout << "  (chs_payload still has sentinel — CHS dispatcher did not fire)"
                << std::endl;
    }
    ++errors;
  }
