...
static void show_usage() {
  std::cout << "Vortex Sgemm TCU SAIF Test." << std::endl;
  std::cout << "Usage: [-m: M] [-n N] [-k: K] [-z sparsity-percent] [-h: help]" << std::endl;
}

static void parse_args(int argc, char **argv) {
  int c;
  while ((c = getopt(argc, argv, "m:n:k:z:i:o:hs")) != -1) {
    switch (c) {
    case 'm':
      xm = atoi(optarg);
      break;
    ...
    case 'k':
      ...
      break;
    case 'z':
      sparsity_percent = atoi(optarg);
      if (sparsity_percent > 100) {
        std::cout << "Error: sparsity percent must be in [0, 100]!" << std::endl;
        exit(-1);
      }
      break;
    case 'h':
...
static bool sparse_zero_at(uint32_t index, uint32_t percent, uint32_t salt) {
  if (percent == 0)
    return false;
  if (percent >= 100)
    return true;
...
static void apply_dual_side_sparsity(itype_t *A, itype_t *B, uint32_t M, uint32_t N, uint32_t K, uint32_t percent) {
  uint32_t subbytes = 8 / vt::ITYPE::bits;
  uint32_t KS = subbytes ? (K * subbytes) : K;
  for (uint32_t i = 0; i < M * KS; ++i) {
    if (sparse_zero_at(i, percent, 0x9e3779b9)) {
      data_accessor_t<vt::ITYPE>::write(A, i, 0);
    }
  }
  for (uint32_t i = 0; i < KS * N; ++i) {
    if (sparse_zero_at(i, percent, 0x85ebca6b)) {
      data_accessor_t<vt::ITYPE>::write(B, i, 0);
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "dual-side sparsity: " << sparsity_percent << "%" << std::endl;
  ...
  apply_dual_side_sparsity(h_A.data(), h_B.data(), M, N, K, sparsity_percent);
