...
int main(int argc, char *argv[]) {
  ...
  cmdbuffer *cmdBuf = vx_create_command_buffer(8);
  ...
  char* test = (char*)calloc(1, 64);
  memcpy(buf_ptr, (void*)&cmdBuf->fifo.at(0), 64);
  ...
  std::cout << "upload kernel argument" << std::endl;
  {
    ...
    vx_new_copy_to_dev(staging_buf, KERNEL_ARG_DEV_MEM_ADDR, sizeof(kernel_arg_t), 0, cmdBuf); // append to command buffer
  ...
  vx_new_copy_to_dev(staging_buf, kernel_arg.src0_addr, buf_size, 0, cmdBuf); // append to command buffer   
  ...
  vx_new_copy_to_dev(staging_buf, kernel_arg.src1_addr, buf_size, 0, cmdBuf);  
  ...
  vx_new_copy_to_dev(staging_buf, kernel_arg.dst_addr, buf_size, 0, cmdBuf);   
