...
__kernel void kernel_main(kernel_arg_t* __UNIFORM__ arg) {
  ...
  for (uint32_t i = 0; i < K; i += ctx::tileK) {
    ...
    auto pTileMxA = pMxA + (blockIdx.y * num_k_tiles + k_tile) * NUM_THREADS;
    auto pTileMxB = pMxB + (blockIdx.x * num_k_tiles + k_tile) * NUM_THREADS;

    ctx::load_matrix_sync(fragA, pTileA, K, pTileMxA);
    ctx::load_matrix_sync<vt::col_major>(fragB, pTileB, K, pTileMxB);
