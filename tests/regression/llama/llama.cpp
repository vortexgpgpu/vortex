...
void matmul(float* xout, float* x, float* w, int n, int d) {
    if(use_vortex) {
        // Use Vortex GPU acceleration for matrix-vector multiplication
        // This performs: W (d,n) @ x (n,) -> xout (d,)
        // We treat x as a (n,1) matrix: vx_matmul(C, A, B, M, N, K) computes C = A * B
        // where A is (M,K), B is (K,N), C is (M,N)
        // So: W (d,n) * x (n,1) = xout (d,1) means M=d, K=n, N=1
        
        vx_matmul(xout, w, x, d, 1, n);
    } else {
        // CPU implementation when vortex is disabled
        // W (d,n) @ x (n,) -> xout (d,)
        for (int i = 0; i < d; i++) {
            float val = 0.0f;
            for (int j = 0; j < n; j++) {
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    if (use_vortex) { vx_legacy_report(pos); }
#endif
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 2; i < argc; i+=2) {
        ...
        else if (argv[i][1] == 'v') { use_vortex = atoi(argv[i + 1]); }
        else { error_usage(); }
    }

    if (use_vortex) {
        printf(">> Using vortex acceleration\n");
        vx_init();
    }
