...
extern "C" void kernel_main(kernel_arg_t *__UNIFORM__ arg) {
  ...
  static constexpr uint32_t a_tile_regs = tile_M * tile_K / i_ratio;
  static constexpr uint32_t b_tile_regs = tile_K * tile_N / i_ratio;
  static constexpr uint32_t tiles_bytes = (a_tile_regs + b_tile_regs) * sizeof(uint32_t);

  static constexpr uint32_t bitmap_tile_regs = tile_K;
  ...
  */
  ...
  uint32_t* B_lmem[2] = {A_lmem[0] + a_tile_regs, A_lmem[1] + a_tile_regs};
...
#pragma unroll
      for (uint32_t tile_col_idx = 0; tile_col_idx < tiles_n; ++tile_col_idx) {
        ...
        if (have_pending_mma && next_stage == next_stage) {
          launch_pending_mma();
        }
        // if (have_running_mma && current_stage == next_stage) {
        //   // tcu_bar[current_stage].arrive_and_wait();
        //   have_running_mma = false;
        // }
...
#pragma unroll
        for (uint32_t dense_iter = 0; dense_iter < kDenseLaunches; ++dense_iter) {
          ...
          // const uint32_t* mma_B_bitmap = reinterpret_cast<const uint32_t *>(B_bitmap_lmem);
          const uint32_t k_tile_idx = k_offset / tile_K;
          // const uint32_t next_stage = dense_iter & 1u;
          const bool use_half0 = (next_stage == 0);
          ...
          if (is_dxa_quad) {
            ...
            if constexpr (kSparseA) {
              ...
              vx_dxa_issue_1d_wg(kDescABitmap, load_bar[next_stage].id(), A_bitmap_lmem, a_bitmap_start);
              vx_dxa_retile_1d_wg(kDescA, a_elems);
              vx_dxa_issue_1d_wg(kDescA, load_bar[next_stage].id(), chunk_A, a_offset);
            } else {
              vx_dxa_retile_2d_wg(kDescA, a_elems, 1);
              vx_dxa_issue_2d_wg(kDescA, load_bar[next_stage].id(), chunk_A, 0, tile_row_tiles_base + k_tile_idx);
            }

            const uint32_t b_bitmap_start = tile_col_idx * K + k_offset;
            vx_dxa_retile_1d_wg(kDescBBitmap, curr_k);
            vx_dxa_issue_1d_wg(kDescBBitmap, load_bar[next_stage].id(), B_bitmap_lmem, b_bitmap_start);
            vx_dxa_retile_1d_wg(kDescB, b_elems);
            vx_dxa_issue_1d_wg(kDescB, load_bar[next_stage].id(), chunk_B, b_offset);
          }
          ...
          //   // tcu_bar[next_stage].arrive_and_wait();
          //   vx_dxa_retile_2d_wg(kDescA, a_elems, 1);
          //   vx_dxa_issue_2d_wg(kDescA, load_bar[next_stage].id(), chunk_A, 0, tile_row_idx * tiles_k + k_tile_idx);
          //   vx_dxa_retile_2d_wg(kDescB, b_elems, 1);
          //   vx_dxa_issue_2d_wg(kDescB, load_bar[next_stage].id(), chunk_B, 0, tile_col_idx * tiles_k + k_tile_idx);
          // }
          ...
          //           (size_t)(uintptr_t)tcu_bar[next_stage].id(),
    ...
    if (have_running_mma) {
      tcu_bar[running_stage].arrive_and_wait();
      have_running_mma = false;
