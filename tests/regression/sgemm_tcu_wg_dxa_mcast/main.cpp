  ...
  for (uint32_t m = 0; m < M; ++m) {
    for (uint32_t n = 0; n < N; ++n) {
      if constexpr (std::is_same<vt::OTYPE, vt::fp32>::value) {
        ...
        // accumulator and rounds to fp32 once; each product is exact in
        // fp32, so a double accumulation reproduces the single-rounding dot
        // product.
        double acc = 0.0;
...
int main(int argc, char *argv[]) {
  ...
  // mc_group_size CTAs co-resident on one core, sharing the same B tile.
  ...
  if (mc_group_size < 2) {
    std::cout << "Warning: NUM_WARPS_per_core (" << num_warps_cap
              << ") too small for mc_group_size >= 2 with " << warps_per_cta
              << "-warp CTAs; multicast degenerates to single receiver\n";
  }
  ...
  // sequentially with stride = per-CTA SMEM size.
