...
// Naked dispatchers reusing the M-mode trap path (see rtu_smoke_ahs_bvh).
...
__kernel void kernel_main(kernel_arg_t* arg) {
  ...
  // Ray flags = 0: the per-instance FORCE_NO_OPAQUE flag alone drives the
  // opacity classification, so the opaque triangle yields an AHS callback.
