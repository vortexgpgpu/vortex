...
int main(int /*argc*/, char* /*argv*/[]) {
  ...
  // One "procedural" primitive — same vertex layout as the smoke
  // triangle, but tri.flags carries PROCEDURAL instead of OPAQUE so
  // the walker yields IS (cb_type=PROC) on hit instead of committing.
  ...
  *tri_flags = RTU_TRI_FLAG_PROC;   // procedural, NOT opaque
  ...
  std::cout << "scene_addr=0x" << std::hex << kernel_arg.scene_addr << std::dec
            ...
            << std::dec << " (procedural primitive, IS path)" << std::endl;
  ...
  // Oracle: ACCEPT path committed the candidate at t=5; CB_TYPE seen
  // by the dispatcher was PROC, so it wrote MAGIC (not ~MAGIC).
  uint32_t exp_status  = VX_RT_STS_DONE_HIT;
  uint32_t exp_payload = RTU_IS_MAGIC;
  std::cout << "oracle: status=" << exp_status
            << " hit_t=5 is_payload=0x" << std::hex << exp_payload
            << std::dec << std::endl;

  int errors = 0;
  bool sts_ok = (result.status == exp_status);
  bool t_ok   = std::fabs(result.hit_t - 5.f) < 1e-4f;
  bool pl_ok  = (result.is_payload == exp_payload);
  if (!sts_ok || !t_ok || !pl_ok) {
    std::cout << "result: status=" << result.status
              << " hit_t=" << result.hit_t
              << " is_payload=0x" << std::hex << result.is_payload
              << std::dec << std::endl;
    if (!pl_ok && result.is_payload == ~RTU_IS_MAGIC) {
      std::cout << "  (dispatcher saw cb_type != PROC)" << std::endl;
    }
    ++errors;
  }
