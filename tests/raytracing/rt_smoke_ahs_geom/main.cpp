...
// which rt_smoke_tlas_builder already covers).
...
static const uint32_t kLeafGeom   = 42u;   // distinctive gl_GeometryIndexEXT
...
int main(int /*argc*/, char* /*argv*/[]) {
  ...
  host_bvh_t blas = { &tri, 1, /*geometry_index*/ kLeafGeom };
  ...
  int errors = 0;
  std::cout << "candidate: geometry=" << cand.cand_geometry
            << " id=" << cand.cand_instance_id << std::endl;
  std::cout << "terminal:  status=" << result.status << " hit_t=" << result.hit_t
            << " geometry=" << result.geometry_index
            << " instance_id=" << result.instance_id << std::endl;
  std::cout << "oracle: candidate geometry=" << kLeafGeom
            << " id=" << kInstId << std::endl;

  // I26: the CANDIDATE stage must carry the candidate leaf's geometry index
  // (gl_GeometryIndexEXT), not the committed hit's (0 mid-walk).
  if (cand.cand_geometry != kLeafGeom) {
    std::cout << "FAIL: candidate geometry mismatch (got "
              << cand.cand_geometry << ")" << std::endl;
    ++errors;
  }
  ...
  // The accepted candidate commits, so the terminal hit reports the same
  // geometry index.
  if (result.status != VX_RT_STS_DONE_HIT)          { std::cout << "FAIL: status\n";   ++errors; }
  if (std::fabs(result.hit_t - 5.f) > 1e-4f)        { std::cout << "FAIL: hit_t\n";    ++errors; }
  if (result.geometry_index != kLeafGeom)           { std::cout << "FAIL: term geometry\n"; ++errors; }
