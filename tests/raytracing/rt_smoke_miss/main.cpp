...
//
// PRISM RTU Miss Shader smoke — Phase 5 host driver.
...
int main(int /*argc*/, char* /*argv*/[]) {
  ...
  // Same triangle as the CHS test, but the ray fires AWAY from it
  // (direction -Z from origin (0.25, 0.25, 0)) so traversal misses.
  ...
  *tri_flags = RTU_TRI_FLAG_OPAQUE;
  ...
  kernel_arg.ray_direction[2] = -1.0f;   // <- fires away from the tri
  ...
  std::cout << "scene_addr=0x" << std::hex << kernel_arg.scene_addr << std::dec
            ...
            << std::dec << " (ray fires -Z, must miss the +Z triangle)"
            << std::endl;
  ...
  uint32_t exp_status  = VX_RT_STS_DONE_MISS;
  uint32_t exp_payload = RTU_MISS_MAGIC;
  std::cout << "oracle: status=" << exp_status
            << " miss_payload=0x" << std::hex << exp_payload << std::dec
            << std::endl;

  int errors = 0;
  bool sts_ok = (result.status == exp_status);
  bool pl_ok  = (result.miss_payload == exp_payload);
  if (!sts_ok || !pl_ok) {
    std::cout << "result: status=" << result.status
              << " miss_payload=0x" << std::hex << result.miss_payload
              << std::dec << std::endl;
    if (!pl_ok && result.miss_payload == 0xdeadbeef) {
      std::cout << "  (miss_payload still has sentinel — MISS dispatcher did not fire)"
                << std::endl;
    }
    ++errors;
  }
