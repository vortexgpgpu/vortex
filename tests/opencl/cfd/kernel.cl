/* CFD (Rodinia euler3d) OpenCL kernels — flux/step solver.
 * Math is kept identical to the original Rodinia Kernels.cl so that the
 * device result matches the serial CPU reference in main.cc. One work-item
 * processes one mesh element; work-group size is therefore free.
 */
#ifndef _KERNEL_
#define _KERNEL_
...
// Initialize every element's flow variables to the far-field state.
...
// Convective + artificial-viscosity flux across each element's NNB faces.
__kernel void compute_flux(__global int* elements_surrounding_elements,
...
// Runge-Kutta update of the flow variables from old_variables + factor*flux.
