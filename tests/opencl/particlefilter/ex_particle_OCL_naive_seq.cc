...
int * ind ;
float * u;
int read_kernel_file_n(const char* filename, uint8_t** data, size_t* size) {
...
float calcLikelihoodSum(int * I, int * ind, int numOnes) {
    ...
    int y;
    for (y = 0; y < numOnes; y++)
        likelihoodSum += (pow((float) (I[ind[y]] - 100), 2) - pow((float) (I[ind[y]] - 228), 2)) / 50.0;
...
*/
    ...
    if (0 != read_kernel_file_n("naive.pocl", &kernel_bin, &kernel_size))
      std::abort();
    ...
    free(kernel_bin);
	  ...
	  err = clBuildProgram(prog, 1, &device_list[device_id_inuse], 
      NULL, NULL, NULL);
	  if(err != CL_SUCCESS) { 
	...
	likelihood = (float *) malloc(sizeof (float) *Nparticles);
	arrayX = (float *) malloc(sizeof (float) *Nparticles);
	arrayY = (float *) malloc(sizeof (float) *Nparticles);
	xj = (float *) malloc(sizeof (float) *Nparticles);
	yj = (float *) malloc(sizeof (float) *Nparticles);
	CDF = (float *) malloc(sizeof (float) *Nparticles);

	ind = (int*) malloc(sizeof (int) *countOnes);
	u = (float *) malloc(sizeof (float) *Nparticles);
...
int particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
  ...
  printf("Nparticles %d\n ", Nparticles);
	...
	for (k = 1; k < Nfr; k++) {
		...
		//OpenCL memory allocation
		cl_int err = clEnqueueWriteBuffer(cmd_queue, arrayX_GPU, 1, 0, sizeof (float) *Nparticles, arrayX, 0, 0, 0);
		...
		err = clEnqueueWriteBuffer(cmd_queue, CDF_GPU, 1, 0, sizeof (float) *Nparticles, CDF, 0, 0, 0);
		if(err != CL_SUCCESS) { printf("ERROR: clEnqueueWriteBuffer CDF_GPU (size:%d) => %d\n", Nparticles, err); return -1; }
		err = clEnqueueWriteBuffer(cmd_queue, u_GPU, 1, 0, sizeof (float) *Nparticles, u, 0, 0, 0);
		...
		err = clEnqueueReadBuffer(cmd_queue, yj_GPU, 1, 0, sizeof (float) *Nparticles, yj, 0, 0, 0);
		if (err != CL_SUCCESS) {
			printf("ERROR: Memcopy Out\n");
			return -1;
		}
		err = clEnqueueReadBuffer(cmd_queue, xj_GPU, 1, 0, sizeof (float) *Nparticles, xj, 0, 0, 0);
		if (err != CL_SUCCESS) {
			printf("ERROR: Memcopy Out\n");
			return -1;
		}
