// Myocyte ODE right-hand-side model (Rodinia OpenCL "myocyte").
//
// This is the host golden reference's copy of kernel_ecc / kernel_cam. The exact
// same source text is inlined into the OpenCL device kernel (kernel.cl); both are
// generated identically from the Rodinia source, so device and host evaluate the
// same model. The address-space qualifier is abstracted through GLOBAL: it
// expands to nothing for this host build and to __global in kernel.cl. `fp`
// (float) is defined by the includer before this header.
//
// Faithful copy of kernel_ecc / kernel_cam from Rodinia, with one bug fix:
// kernel_cam originally wrote its inter-model Ca-flux value to
// d_finavalu[com_offset] (clobbering ECC gating-variable derivatives and
// leaving the d_com buffer uninitialised, which master.c then read back). The
// clear intent (see master.c -> kernel_fin using com[0..2] as JCaDyad/SL/Cyt)
// is d_com[com_offset]; fixed accordingly so device and host agree.

#ifndef MYOCYTE_MODEL_H
#define MYOCYTE_MODEL_H
