// Myocyte (Rodinia) OpenCL kernel — standalone Vortex port.
//
// Evaluates the cardiac-myocyte ODE right-hand side (91 equations) for one time
// instant. Work-group 0 runs the EC-coupling model (kernel_ecc, states 0..45);
// work-group 1 runs the CaM/CaMKII/CaN model three times (kernel_cam, states
// 46..90 for the Dyad/SL/Cyt compartments). Launched with global=2*NUMBER_THREADS
// (=4) and local=NUMBER_THREADS (=2): 2 work-groups, well under the device's
// max work-group size of NUM_WARPS*NUM_THREADS = 16.
//
// The kernel_ecc / kernel_cam bodies below are shared verbatim with the host
// golden reference in myocyte_model.h (same source, GLOBAL abstracts the address
// space). Two fixes vs stock Rodinia: (1) kernel_cam writes its inter-model
// Ca-flux to d_com[] not d_finavalu[] (stock clobbered ECC derivatives and left
// d_com uninitialised; master.c's intent via kernel_fin is d_com[com_offset]);
// (2) the kernel `timeinst` argument is float — stock declared it int while the
// host passed a float's bit pattern (a reinterpret bug); it is passed as float.

#define fp float
#define GLOBAL __global

//========================================================================================
//	MODEL (kernel_ecc + kernel_cam) — identical to host myocyte_model.h
//========================================================================================

void 
kernel_ecc(	fp timeinst,
			GLOBAL fp *d_initvalu,
			GLOBAL fp *d_finavalu,
			int valu_offset,
			GLOBAL fp *d_params){
...
__kernel void 
kernel_gpu_opencl(	float timeinst,
