...
*/
  uint8_t *kernel_bin = NULL;
    ...
    program = clCreateProgramWithBinary(
          context, 1, &cldevice, &kernel_size,
          (const uint8_t**)&kernel_bin, &binary_status, &errNum);
    free(kernel_bin);
...
///
//in file dwt.cu
void samplesToChar_(unsigned char * dst, int * src, int samplesNum)
{
    int i;
...
int writeLinear(cl_mem component, int pixWidth, int pixHeight, const char * filename, const char * suffix)
{
	...
	// T to char 
	samplesToChar_(result, gpu_output, samplesNum);
...
int writeNStage2DDWT(cl_mem component, int pixWidth, int pixHeight, int stages, const char * filename, const char * suffix)
{
    ...
    // Write component
	samplesToChar_(result, dst, samplesNum);	
...
int main(int argc, char **argv) 
{
    ...
    */
    ...
    int pixWidth    = 16; //<real pixWidth
    int pixHeight   = 16; //<real pixHeight
    int compCount   = 3; //number of components; 3 for RGB or YUV, 4 for RGBA
    int bitDepth    = 8; 
    int dwtLvls     = 1; //default numuber of DWT levels
...
*/
	...
	{
        printf("Wrong or missing dimensions %d %d\n", pixWidth, pixHeight);
        usage();
        return -1;
    }
	...
	// file names
    d->srcFilename = "16.bmp"; // (char *)malloc(strlen(argv[0]));
