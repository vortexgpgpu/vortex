...
{
   const char *spv_path = (argc > 1) ? argv[1] : "rtquery_anyhit.comp.spv";

   /* --- instance --------------------------------------------------- */
   VkApplicationInfo app = {
      .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "vortexpipe-rtquery_anyhit",
   ...
   VkAccelerationStructureGeometryKHR tri_geom = {
      ...
      .flags = 0,   /* non-opaque: candidates go to the shader's any-hit test */
      .geometry.triangles = {
   ...
   vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pl,
                           0, 1, &ds, 0, NULL);
   vkCmdDispatch(cmd, WIDTH / 4, HEIGHT / 4, 1);
   ...
   bool ok = bad == 0 && corner == MISS_COLOR &&
             hits > 30u && hits < 450u;
   if (!ok) {
      printf("FAILED (hits=%u misses=%u bad=%u corner=0x%08x)\n",
