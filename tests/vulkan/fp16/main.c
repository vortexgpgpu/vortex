 ...
 *
 * fp16 coverage test for the vortexpipe driver.
 *
 * Dispatches fp16.comp, which runs binary16 arithmetic and converts over
 * per-invocation inputs, and reproduces every result here on the host. All
 * operands and results are exactly representable in fp16, so the comparison
 * is exact -- no tolerance to hide a rounding regression in.
 *
 * shaderFloat16 must be queried and enabled: it is inherited from llvmpipe
 * and never overridden by vortexpipe, so if the device cannot execute fp16
 * this test is how that surfaces.
 *
 * The buffer is seeded with a sentinel rather than left undefined: under
 * MESA_VORTEX_STRICT=1 a translator bail turns the dispatch into a no-op, so
 * an op the device cannot lower leaves its sentinel behind and fails here.
 * That is what makes a pass evidence the *device* computed these ops.
 */
...
/* ---- host reference for fp16.comp ------------------------------------
 * Every value below is exact in binary16 and therefore exact in binary32,
 * so the readback comparison is ==. See fp16.comp for why denormals are out
 * of scope here. */

static const char *const op_name[] = {
   "add", "sub", "mul2", "min", "max", "abs", "i2f16", "half_sq",
};

static void
fp16_expect(uint32_t i, float *out)
{
   const float a = 1.5f + (float)i;
   const float b = 2.25f;
...
main(int argc, char **argv)
{
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "vortexpipe-fp16-smoke",
   ...
   /* shaderFloat16 is optional. Query it before creating the device: if the
    * implementation does not offer it, say so and exit 0 -- an honest skip.
    * Reporting a pass here would be worse than useless, because the whole
    * point of this test is that fp16 executes on the device. */
   ...
   if (!f16.shaderFloat16) {
      printf("NotSupported: shaderFloat16 is not offered by this device\n");
      return 0;
   }

   VkPhysicalDeviceShaderFloat16Int8Features f16_en = {
      .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SHADER_FLOAT16_INT8_FEATURES,
   ...
   /* seed every slot with the sentinel so a no-op dispatch is detectable */
   float *p;
   CHECK(vkMapMemory(dev, mem, 0, bytes, 0, (void **)&p));
   for (uint32_t i = 0; i < N; i++) p[i] = SENTINEL;
   ...
   vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_COMPUTE, pl,
                           0, 1, &ds, 0, NULL);
   vkCmdDispatch(cb, (INVOCS + local_size - 1u) / local_size, 1, 1);
   ...
   /* --- read back + verify ---------------------------------------- */
   CHECK(vkMapMemory(dev, mem, 0, bytes, 0, (void **)&p));
   unsigned fails = 0, sentinels = 0;
   for (uint32_t i = 0; i < INVOCS; i++) {
      float want[NUM_OPS];
      fp16_expect(i, want);
      for (uint32_t k = 0; k < NUM_OPS; k++) {
         float got = p[i * NUM_OPS + k];
         if (got == SENTINEL) {
            sentinels++;
         }
         if (got != want[k]) {
            if (fails < 8)
               fprintf(stderr, "  invoc %u %s: got %g, want %g\n",
                       i, op_name[k], got, want[k]);
            fails++;
         }
      }
   }
   ...
   if (sentinels)
      fprintf(stderr,
              ...
              "write them. Under STRICT this means the shader bailed to "
              "llvmpipe: an op in alu.comp has no lowering in the translator.\n",
              sentinels, (unsigned)N);
   ...
   printf("PASSED (%u invocations x %u fp16 ops)\n", INVOCS, NUM_OPS);
   return 0;
}
