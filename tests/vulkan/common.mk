...
ifeq ($(TARGET), hw)
	@out=`SCOPE_JSON_PATH=$(FPGA_BIN_DIR)/scope.json XRT_INI_PATH=$(VORTEX_RT_SRC)/xrt/xrt.ini EMCONFIG_PATH=$(FPGA_BIN_DIR) XRT_DEVICE_INDEX=$(XRT_DEVICE_INDEX) XCLBIN_PATH=$(FPGA_BIN_DIR)/vortex_afu.xclbin $(RUN_ENV) LD_LIBRARY_PATH=$(XILINX_XRT)/lib:$(RUN_LD_PATH) VORTEX_DRIVER=xrt ./$(PROJECT) $(SPVS) $(OPTS) 2>&1`; \
	$(check_run)
else ifeq ($(TARGET), hw_emu)
	@out=`SCOPE_JSON_PATH=$(FPGA_BIN_DIR)/scope.json XCL_EMULATION_MODE=$(TARGET) XRT_INI_PATH=$(VORTEX_RT_SRC)/xrt/xrt.ini EMCONFIG_PATH=$(FPGA_BIN_DIR) XRT_DEVICE_INDEX=$(XRT_DEVICE_INDEX) XCLBIN_PATH=$(FPGA_BIN_DIR)/vortex_afu.xclbin $(RUN_ENV) LD_LIBRARY_PATH=$(XILINX_XRT)/lib:$(RUN_LD_PATH) VORTEX_DRIVER=xrt ./$(PROJECT) $(SPVS) $(OPTS) 2>&1`; \
	$(check_run)
else
