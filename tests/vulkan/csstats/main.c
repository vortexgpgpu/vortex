 ...
 *
 * A pipeline-statistics query over a dispatch the device runs.
 *
 * The compute-invocation counter is llvmpipe's, incremented as it executes the
 * dispatch, and a dispatch that takes the device path never reaches it -- so
 * the query returns zero with no error and no fallback. This is the compute
 * sibling of the occlusion-query defect: the same class of counter lost below
 * the same kind of replaced entry point, one path over.
 *
 * The dispatch itself is the private-array kernel, whose result is checked
 * first -- a dispatch that went wrong is reported as that rather than blamed
 * on the query.
 */
...
/* Must match SLOTS in csstats.comp. */
#define SLOTS       64u
...
{
   const char *spv_path = (argc > 1) ? argv[1] : "csstats.comp.spv";

   VkApplicationInfo app = {
      .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "vortexpipe-csstats",
   ...
   /* Pipeline-statistics queries are an opt-in core feature. */
   VkPhysicalDeviceFeatures feats = { .pipelineStatisticsQuery = VK_TRUE };
    ...
    * down to the largest power of two that divides N. csstats.comp
   ...
   vkCmdBeginQuery(cb, qp, 0, 0);
   vkCmdDispatch(cb, N / local_size, 1, 1);
   vkCmdEndQuery(cb, qp, 0);
   ...
   for (uint32_t i = 0; i < N; i++) {
      /* The shader stored k at slot (k + i) % SLOTS, so the slot it reads back
       * holds the k that landed there: (slot - i) taken positive. */
      uint32_t slot = (i * 7u) % SLOTS;
      uint32_t want = (slot + SLOTS - (i % SLOTS)) % SLOTS;
      if (p[i] != want) {
         if (fails < 5)
   ...
   /* WAIT so the count is the finished one rather than a partial read. */
   uint64_t invocations = 0;
   CHECK(vkGetQueryPoolResults(dev, qp, 0, 1, sizeof invocations, &invocations,
                               sizeof invocations,
   ...
   if (fails) {
      printf("FAILED (%u/%u mismatches, so the query result cannot be "
             "judged)\n", fails, N);
      return 1;
   }
   if (invocations != (uint64_t)N) {
      printf("FAILED (csstats: query counted %llu compute invocations over a "
             "dispatch of %u; zero means the counter was never updated, which "
             "is what happens when the dispatch bypasses the executor that "
             "owns it)\n", (unsigned long long)invocations, N);
      return 1;
   }
   printf("PASSED (csstats: query counted %llu compute invocations over a "
          "dispatch of %u)\n", (unsigned long long)invocations, N);
   return 0;
}
