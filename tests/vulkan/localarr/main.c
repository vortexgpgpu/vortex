 ...
 *
 * Private-array compute test for the vortexpipe driver.
 *
 * Each invocation fills a 64-slot private array, rotating the destination by
 * its own index, then reads one slot back at a second runtime-computed index.
 * Both indices depend on the invocation, so neither folds away and the array
 * stays a real indirect access rather than a select chain over SSA values.
 *
 * The expected value inverts the rotation, so a store index that is dropped,
 * a load index that is dropped, and a rotation applied in the wrong direction
 * each produce a different wrong answer.
 */
...
/* Must match SLOTS in localarr.comp. */
#define SLOTS       64u
...
{
   const char *spv_path = (argc > 1) ? argv[1] : "localarr.comp.spv";

   VkApplicationInfo app = {
      .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "vortexpipe-localarr",
    ...
    * down to the largest power of two that divides N. localarr.comp
   ...
   printf("PASSED (%u invocations, %u-slot private array indexed both ways)\n",
          N, SLOTS);
   return 0;
}
