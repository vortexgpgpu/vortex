 ...
 *
 * Logic-op test for the vortexpipe driver.
 *
 * The target is cleared to yellow and a full-screen quad emits magenta
 * through an XOR logic op, so every pixel merges to cyan -- a value neither
 * operand carries. Every channel is 0 or 255, so the merge is exact rather
 * than a tolerance. The blend factors are left at their defaults, which the
 * output merger reads as "blending disabled": a logic op has to take effect
 * anyway, which is what this case pins down.
 */
...
{
   const char *vs_path = (argc > 1) ? argv[1] : "logicop.vert.spv";
   const char *fs_path = (argc > 2) ? argv[2] : "logicop_xor.frag.spv";

   /* --- instance --------------------------------------------------- */
   VkApplicationInfo app = {
      .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "vortexpipe-logicop",
   ...
   VkPhysicalDeviceFeatures feats = { .logicOp = VK_TRUE };
   ...
   VkPipelineColorBlendStateCreateInfo cb = {
      ...
      .logicOpEnable = VK_TRUE, .logicOp = VK_LOGIC_OP_XOR,
   ...
   VkClearValue clear = { .color = { .float32 = { 1.0f, 1.0f, 0.0f, 1.0f } } };
   ...
   /* --- read back + verify: every pixel is the XOR of the two ------- */
   uint8_t *px;
   CHECK(vkMapMemory(dev, bmem, 0, bytes, 0, (void **)&px));
   unsigned cyan = 0, wrong = 0;
   for (uint32_t i = 0; i < WIDTH * HEIGHT; i++) {
      const uint8_t *p = px + (size_t)i * 4;
      /* magenta (255,0,255) XOR yellow (255,255,0) = cyan (0,255,255) */
      if (p[0] == 0 && p[1] == 255 && p[2] == 255) cyan++;
      else wrong++;
   }
   vkUnmapMemory(dev, bmem);

   bool ok = wrong == 0 && cyan == WIDTH * HEIGHT;
   ...
   if (!ok) {
      ...
      printf("FAILED (logic op XOR: %u/%u pixels cyan; centre = %u,%u,%u)\n",
             cyan, WIDTH * HEIGHT, c[0], c[1], c[2]);
      return 1;
   }
   printf("PASSED (logic op XOR: %u pixels merged to cyan)\n", cyan);
   return 0;
}
