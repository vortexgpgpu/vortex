 ...
 *
 * textureGather test for the vortexpipe driver.
 *
 * Three cases, each a full-screen quad over a 32x32 target whose fragment
 * shader gathers a 2x2 texel footprint, checks the four values and their
 * GL order in-shader, and writes green on match / red on mismatch:
 *
 *   gather_comp   -- 4x4 RGBA8, all four component selects
 *   gather_wrap   -- footprint crossing the edge under CLAMP_TO_EDGE
 *   gather_array  -- 4x4x2 array, layer 1 (layer 0 is a decoy grey)
 *
...
main(int argc, char **argv)
{
   ...
   struct texcase cases[] = {
      { "gather_comp", "gather_comp.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 1, VK_IMAGE_VIEW_TYPE_2D, rgba8_texels, sizeof(rgba8_texels) },
      { "gather_wrap", "gather_wrap.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 1, VK_IMAGE_VIEW_TYPE_2D, rgba8_texels, sizeof(rgba8_texels) },
      { "gather_array", "gather_array.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
   ...
   const char *vs_path = (argc > 1) ? argv[1] : "tex_gather.vert.spv";
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "vortexpipe-tex_gather",
   ...
   printf("PASSED (textureGather: all cases green)\n");
   return 0;
}
