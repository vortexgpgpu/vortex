 ...
 *
 * texelFetch / textureSize test for the vortexpipe driver.
 *
 * Four cases, each a full-screen quad over a 32x32 target whose
 * fragment shader fetches texels at integer coordinates, verifies them
 * in-shader against the values the host uploaded, and writes green on
 * match / red on mismatch:
 *
 *   fetch_rgba8  -- 4x4 RGBA8, per-texel pattern + textureSize
 *   fetch_f32    -- 2x2 RGBA32F with values outside [0,1] (float carrier)
 *   fetch_i8     -- 2x2 RGBA8_SINT via isampler2D (integer carrier)
 *   fetch_array  -- 4x4x2 RGBA8 array, layer 1 + textureSize layer count
 *
 * A case passes when every rendered pixel is green.
 */
...
/* Per-texel patterns mirrored by the fragment shaders. */
static uint8_t rgba8_texels[4 * 4][4];
static float   f32_texels[2 * 2][4];
static int8_t  i8_texels[2 * 2][4];
static uint8_t array_texels[2][4 * 4][4];
...
fill_patterns(void)
{
   for (uint32_t y = 0; y < 4; y++)
      for (uint32_t x = 0; x < 4; x++) {
         uint8_t *p = rgba8_texels[y * 4 + x];
         p[0] = (uint8_t)(x * 60 + 15);
         p[1] = (uint8_t)(y * 60 + 15);
         p[2] = 200; p[3] = 255;
         uint8_t *l0 = array_texels[0][y * 4 + x];
         l0[0] = 30; l0[1] = 30; l0[2] = 30; l0[3] = 255;
         uint8_t *l1 = array_texels[1][y * 4 + x];
         l1[0] = 0;
         l1[1] = (uint8_t)(x * 60 + 15);
         l1[2] = (uint8_t)(y * 60 + 15);
         l1[3] = 255;
      }
   for (uint32_t y = 0; y < 2; y++)
      for (uint32_t x = 0; x < 2; x++) {
         float s = (float)(x + 2 * y + 1);
         float *f = f32_texels[y * 2 + x];
         f[0] = 2.5f * s; f[1] = -1.0f * s; f[2] = 0.25f * s; f[3] = 1.0f * s;
         int8_t *i = i8_texels[y * 2 + x];
         i[0] = (int8_t)((int)x * 10 - 5);
         i[1] = (int8_t)((int)y * 20 - 7);
         i[2] = 42; i[3] = -1;
      }
}
...
main(int argc, char **argv)
{
   ...
   struct texcase cases[] = {
      { "fetch_rgba8", "fetch_rgba8.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 1, VK_IMAGE_VIEW_TYPE_2D, rgba8_texels, sizeof(rgba8_texels) },
      { "fetch_f32", "fetch_f32.frag.spv", VK_FORMAT_R32G32B32A32_SFLOAT,
        2, 2, 1, VK_IMAGE_VIEW_TYPE_2D, f32_texels, sizeof(f32_texels) },
      { "fetch_i8", "fetch_i8.frag.spv", VK_FORMAT_R8G8B8A8_SINT,
        2, 2, 1, VK_IMAGE_VIEW_TYPE_2D, i8_texels, sizeof(i8_texels) },
      { "fetch_array", "fetch_array.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 2, VK_IMAGE_VIEW_TYPE_2D_ARRAY, array_texels, sizeof(array_texels) },
   };
   const uint32_t ncases = (uint32_t)(sizeof(cases) / sizeof(cases[0]));

   const char *vs_path = (argc > 1) ? argv[1] : "tex_fetch.vert.spv";
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "vortexpipe-tex_fetch",
   ...
   for (uint32_t ci = 0; ci < ncases; ci++) {
      ...
      /* Each case releases its texture before the next one allocates, so a
       * later case's storage can land on a freed address -- the aliasing a
       * descriptor-to-resource match has to stay correct across. */
   ...
   printf("PASSED (texelFetch/textureSize: all cases green)\n");
   return 0;
}
