 ...
 *
 * Samples layer 0 of a three-layer array texture. Each layer is a
...
void main()
{
   vec4 t = texture(tex, vec3(v_uv, float(0)));
   /* layer L is (40*L+40, 20*L+10, 200-60*L) / 255. */
   vec4 want = vec4(float(40 * 0 + 40) / 255.0,
                    float(20 * 0 + 10) / 255.0,
                    float(200 - 60 * 0) / 255.0, 1.0);
