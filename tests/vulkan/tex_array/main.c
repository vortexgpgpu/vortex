 ...
 *
 * 2D-array texture test for the vortexpipe driver.
 *
 * A three-layer 4x4 array whose layers are distinct solid colours, sampled
 * over a full-screen quad on a 32x32 target. Each case checks its colour
 * in-shader and writes green on match / red on mismatch:
 *
 *   array_layer0  -- selects layer 0
 *   array_layer2  -- selects the last layer
 *   array_clamp   -- a layer coordinate past the last, which Vulkan clamps
 *
...
/* Three solid layers, mirrored by the fragment shaders. */
static uint8_t layer_texels[3][4 * 4][4];
...
fill_patterns(void)
{
   for (uint32_t l = 0; l < 3; l++)
      for (uint32_t i = 0; i < 16; i++) {
         uint8_t *p = layer_texels[l][i];
         p[0] = (uint8_t)(40 * l + 40);
         p[1] = (uint8_t)(20 * l + 10);
         p[2] = (uint8_t)(200 - 60 * l);
         p[3] = 255;
      }
}
...
main(int argc, char **argv)
{
   ...
   struct texcase cases[] = {
      { "array_layer0", "array_layer0.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 3, VK_IMAGE_VIEW_TYPE_2D_ARRAY, layer_texels, sizeof(layer_texels) },
      { "array_layer2", "array_layer2.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 3, VK_IMAGE_VIEW_TYPE_2D_ARRAY, layer_texels, sizeof(layer_texels) },
      { "array_clamp", "array_clamp.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 3, VK_IMAGE_VIEW_TYPE_2D_ARRAY, layer_texels, sizeof(layer_texels) },
   };
   const uint32_t ncases = (uint32_t)(sizeof(cases) / sizeof(cases[0]));

   const char *vs_path = (argc > 1) ? argv[1] : "tex_array.vert.spv";
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "vortexpipe-tex_array",
   ...
   printf("PASSED (2D array: all cases green)\n");
   return 0;
}
