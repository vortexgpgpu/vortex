 ...
 *
 * UBO + push-constant Vulkan test for the vortexpipe driver (W7).
 *
 * Renders one triangle off-screen into a 64x64 RGBA8 image. The
 * fragment shader takes its colour entirely from a uniform buffer
 * (set 0, binding 0) scaled by a push constant, so a correct render
 * proves the on-device fragment-shader descriptor path (UBO read +
 * push-constant read) — not vertex interpolation. The centre pixel
 * must equal ubo.color.rgb * pc.scale.
 *
...
/* UBO colour and push-constant scale — the expected centre pixel is
 * (color.rgb * scale) quantized to 8-bit UNORM. */
static const float UBO_COLOR[4] = { 0.8f, 0.4f, 0.2f, 1.0f };
static const float PC_SCALE     = 0.5f;
...
{
   const char *vs_path = (argc > 1) ? argv[1] : "ubo.vert.spv";
   const char *fs_path = (argc > 2) ? argv[2] : "ubo.frag.spv";

   /* --- instance --------------------------------------------------- */
   VkApplicationInfo app = {
      .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "vortexpipe-ubo",
   ...
   VkBufferCreateInfo ubci = {
      ...
      .size = sizeof(UBO_COLOR),
      .usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
   };
   VkBuffer ubo;
   CHECK(vkCreateBuffer(dev, &ubci, NULL, &ubo));
   ...
   if (umt == UINT32_MAX) { fprintf(stderr, "FAILED: no UBO memory\n"); return 1; }
   ...
   {
      void *p;
      CHECK(vkMapMemory(dev, umem, 0, sizeof(UBO_COLOR), 0, &p));
      memcpy(p, UBO_COLOR, sizeof(UBO_COLOR));
      vkUnmapMemory(dev, umem);
   }

   /* --- descriptor set: one uniform buffer ------------------------ */
   ...
   VkDescriptorBufferInfo dbi = {
      .buffer = ubo, .offset = 0, .range = sizeof(UBO_COLOR),
   };
   ...
   /* --- pipeline layout: set 0 + a 4-byte fragment push constant --- */
   VkPushConstantRange pcr = {
      .stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT,
      .offset = 0, .size = sizeof(float),
   };
   ...
   vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pl,
                           0, 1, &dset, 0, NULL);
   float scale = PC_SCALE;
   vkCmdPushConstants(cmd, pl, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                      sizeof(scale), &scale);
   ...
   /* expected centre pixel = ubo.color.rgb * scale, 8-bit UNORM. */
   uint8_t exp[3];
   for (int c = 0; c < 3; c++) {
      float v = UBO_COLOR[c] * PC_SCALE;
      int q = (int)lroundf(v * 255.0f);
   ...
   printf("centre=(%u,%u,%u) expected=(%u,%u,%u) colored=%u\n",
          centre[0], centre[1], centre[2], exp[0], exp[1], exp[2], colored);
   ...
   if (!centre_ok || !corner_clear || !coverage_ok) {
      printf("FAILED (centre_ok=%d corner_clear=%d coverage_ok=%d)\n",
             centre_ok, corner_clear, coverage_ok);
      return 1;
   }
   printf("PASSED (UBO+push-constant fragment colour correct)\n");
   return 0;
}
