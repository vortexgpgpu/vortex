 ...
 *
 * Mip-level / LOD test for the vortexpipe driver.
 *
 * An 8x8 texture with a four-level mip chain, each level a distinct solid
 * colour, sampled over a full-screen quad on a 32x32 target:
 *
 *   lod_base       -- explicit textureLod at level 0
 *   lod_level2     -- explicit textureLod at level 2
 *   lod_trilinear  -- a fractional LOD, which must blend levels 0 and 1
 *
 * The mip chain is what makes the level select observable: a wrong level or
 * a dropped LOD fraction returns a different colour. A case passes when
 * every rendered pixel is green.
 */
...
#define TEX_DIM    8u
#define TEX_LEVELS 4u

/* One solid colour per mip level, mirrored by the fragment shaders. Packed
 * back to back, smallest level last, exactly as the copy regions expect. */
static uint8_t mip_texels[(64 + 16 + 4 + 1) * 4];

static const uint8_t level_rgb[TEX_LEVELS][3] = {
...
fill_patterns(void)
{
   uint8_t *p = mip_texels;
   for (uint32_t l = 0; l < TEX_LEVELS; l++) {
      uint32_t dim = TEX_DIM >> l;
...
main(int argc, char **argv)
{
   ...
   struct texcase cases[] = {
      { "lod_base", "lod_base.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        TEX_DIM, TEX_DIM, 1, VK_IMAGE_VIEW_TYPE_2D,
        mip_texels, sizeof(mip_texels) },
      { "lod_level2", "lod_level2.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        TEX_DIM, TEX_DIM, 1, VK_IMAGE_VIEW_TYPE_2D,
        mip_texels, sizeof(mip_texels) },
      { "lod_trilinear", "lod_trilinear.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        TEX_DIM, TEX_DIM, 1, VK_IMAGE_VIEW_TYPE_2D,
        mip_texels, sizeof(mip_texels) },
   };
   const uint32_t ncases = (uint32_t)(sizeof(cases) / sizeof(cases[0]));

   const char *vs_path = (argc > 1) ? argv[1] : "tex_lod.vert.spv";
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "vortexpipe-tex_lod",
   ...
   /* Mip-linear across the whole chain: the fractional-LOD case needs the
    * blend, and maxLod above 0.5 is also what tells the driver this sampler
    * is mipmapped at all. */
   ...
   for (uint32_t ci = 0; ci < ncases; ci++) {
      ...
      VkImageCreateInfo tci = {
         ...
         .extent = { c->w, c->h, 1 }, .mipLevels = TEX_LEVELS, .arrayLayers = c->layers,
         .samples = VK_SAMPLE_COUNT_1_BIT, .tiling = VK_IMAGE_TILING_OPTIMAL,
      ...
      VkImageViewCreateInfo tvci = {
         ...
         .subresourceRange = {
            .aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
            .levelCount = TEX_LEVELS, .layerCount = c->layers,
         },
      };
      VkImageView texview;
      ...
      VkImageMemoryBarrier to_dst = {
         ...
         .image = tex,
         .subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, TEX_LEVELS, 0, c->layers },
      ...
      VkBufferImageCopy tcopy[TEX_LEVELS];
      VkDeviceSize lvl_off = 0;
      for (uint32_t l = 0; l < TEX_LEVELS; l++) {
         uint32_t dim = TEX_DIM >> l;
         tcopy[l] = (VkBufferImageCopy){
            .bufferOffset = lvl_off,
            .imageSubresource = { .aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
                                  .mipLevel = l, .layerCount = c->layers },
            .imageExtent = { dim, dim, 1 },
         };
         lvl_off += (VkDeviceSize)dim * dim * 4;
      }
   ...
   printf("PASSED (mip LOD: all cases green)\n");
   return 0;
}
