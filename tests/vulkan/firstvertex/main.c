 ...
 *
 * gl_VertexIndex / firstVertex test for the vortexpipe driver.
 *
 * Draws six vertices at firstVertex = 6 from a vertex-less pipeline whose
 * shader derives its geometry from gl_VertexIndex alone. Vulkan defines that
 * index as firstVertex + i, so the draw must fill the right half of the target
 * and leave the left cleared; an index that drops firstVertex fills the left
 * half, which the check reports as the failure it is.
 */
...
{
   const char *vs_path  = (argc > 1) ? argv[1] : "firstvertex.vert.spv";
   const char *fsn_path = (argc > 2) ? argv[2] : "firstvertex.frag.spv";

   /* --- instance + device ----------------------------------------- */
   VkApplicationInfo app = {
      .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "vortexpipe-firstvertex", .apiVersion = VK_API_VERSION_1_1,
   };
   ...
   /* Nothing here should reject a fragment: the target's two halves report
    * which vertices the draw selected, so depth and stencil must not filter. */
   ...
   VkPipelineDepthStencilStateCreateInfo ds = {
      ...
      .stencilTestEnable = VK_FALSE, .front = sop, .back = sop,
   };
   ...
   vkCmdDraw(cmd, 6, 1, 6, 0);   /* gl_VertexIndex must run 6..11 */
   ...
   /* --- verify: the right half is filled, the left is left cleared ----- */
   uint8_t *px;
   CHECK(vkMapMemory(dev, bmem, 0, bytes, 0, (void **)&px));
   unsigned right_drawn = 0, left_drawn = 0;
   for (uint32_t y = 0; y < HEIGHT; y++) {
      for (uint32_t x = 0; x < WIDTH; x++) {
         const uint8_t *p = px + ((size_t)y * WIDTH + x) * 4;
         const bool drawn = p[1] > 200 && p[0] < 80;
         if (!drawn) {
            continue;
         }
         if (x >= WIDTH / 2) {
   ...
   /* Edge pixels along the split can land either side, so require the halves
    * to be decisively right rather than exactly so. */
   const unsigned half = (WIDTH / 2) * HEIGHT;
   const bool ok = right_drawn > half - half / 8 && left_drawn < half / 8;
   ...
   if (!ok) {
      printf("FAILED (firstvertex: %u pixels drawn right, %u left, of %u per half"
             " -- %s)\n", right_drawn, left_drawn, half,
             left_drawn > right_drawn ? "the draw rendered vertices 0-5, so "
                                        "gl_VertexIndex dropped firstVertex"
                                      : "mixed result");
      return 1;
   }
   printf("PASSED (firstvertex: %u pixels drawn in the right half, %u in the left)\n",
          right_drawn, left_drawn);
   return 0;
}
