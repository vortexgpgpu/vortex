 ...
 *
 * What a compute shader can ask about its own dispatch.
 *
 * gl_NumWorkGroups and an unsized storage array's .length() are both
 * implemented on the device -- the first from a CSR, the second out of the
 * descriptor's size field -- and nothing in this suite read either one, so both
 * were carried entirely by never being asked for. They are checked together
 * because they fail the same way: a shader that cannot see its dispatch reads a
 * plausible small number rather than an obviously broken one.
 *
 * The grid is deliberately not 1: a dispatch of a single workgroup would let a
 * gl_NumWorkGroups that always returned 1 pass. For the same reason the element
 * count is not equal to the grid, so neither value can stand in for the other.
 *
...
main(int argc, char **argv)
{
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "vortexpipe-dispatchq",
   ...
   /* Seeded with a value the shader never writes, so a word that was not
    * written is not mistaken for one that was written wrongly. */
   uint32_t *p;
   CHECK(vkMapMemory(dev, mem, 0, bytes, 0, (void **)&p));
   for (uint32_t i = 0; i < N; i++) { p[i] = 0xFFFFFFFFu; }
   ...
   /* --- read back + verify ---------------------------------------- */
   const uint32_t grid = N / local_size;
   const uint32_t expect = grid + 1000u * N;
   CHECK(vkMapMemory(dev, mem, 0, bytes, 0, (void **)&p));
   unsigned fails = 0;
   uint32_t got0 = p[0];
   ...
   if (fails) {
      printf("FAILED (dispatchq: %u/%u wrong; grid read as %u, expected %u; "
             "length read as %u, expected %u)\n", fails, N,
             got0 % 1000u, grid, got0 / 1000u, N);
      return 1;
   }
   printf("PASSED (dispatchq: gl_NumWorkGroups.x=%u over %u invocations, "
          "buffer length()=%u)\n", grid, N, N);
   return 0;
}
