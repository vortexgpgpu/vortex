 ...
 *
 * 3D (volume) texture test for the vortexpipe driver.
 *
 * A 4x4x4 volume whose four depth slices are distinct solid colours,
 * sampled over a full-screen quad on a 32x32 target:
 *
 *   vol_slice0  -- the first slice
 *   vol_slice3  -- the last slice
 *
 * A volume is not an array: the r coordinate is normalized over the depth
 * rather than an integer index, so a slice select that treats it as a layer
 * lands on the wrong colour. A case passes when every pixel is green.
 */
...
struct texcase {
   ...
   const void *data;
   VkDeviceSize bytes;
   uint32_t     depth;          /* 3D slice count */
};

/* Four solid depth slices, mirrored by the fragment shaders. */
static uint8_t slice_texels[4][4 * 4][4];
...
fill_patterns(void)
{
   for (uint32_t z = 0; z < 4; z++)
      for (uint32_t i = 0; i < 16; i++) {
         uint8_t *p = slice_texels[z][i];
         p[0] = (uint8_t)(50 * z + 50);
         p[1] = (uint8_t)(30 * z + 20);
         p[2] = (uint8_t)(200 - 40 * z);
         p[3] = 255;
      }
}
...
main(int argc, char **argv)
{
   ...
   struct texcase cases[] = {
      { "vol_slice0", "vol_slice0.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 1, VK_IMAGE_VIEW_TYPE_3D, slice_texels, sizeof(slice_texels), 4 },
      { "vol_slice3", "vol_slice3.frag.spv", VK_FORMAT_R8G8B8A8_UNORM,
        4, 4, 1, VK_IMAGE_VIEW_TYPE_3D, slice_texels, sizeof(slice_texels), 4 },
   };
   const uint32_t ncases = (uint32_t)(sizeof(cases) / sizeof(cases[0]));

   const char *vs_path = (argc > 1) ? argv[1] : "tex_3d.vert.spv";
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "vortexpipe-tex_3d",
   ...
   for (uint32_t ci = 0; ci < ncases; ci++) {
      ...
      VkImageCreateInfo tci = {
         .sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
         .imageType = VK_IMAGE_TYPE_3D, .format = c->fmt,
         .extent = { c->w, c->h, c->depth }, .mipLevels = 1,
         .arrayLayers = c->layers,
         .samples = VK_SAMPLE_COUNT_1_BIT, .tiling = VK_IMAGE_TILING_OPTIMAL,
   ...
   printf("PASSED (3D volume: all cases green)\n");
   return 0;
}
