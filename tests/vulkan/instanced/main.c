 ...
 *
 * W8 draw-instancing Vulkan test for the vortexpipe driver.
 *
 * Renders a 3-instance triangle draw off-screen into a 64x64 RGBA8
 * image. The vertex shader offsets each instance in X by
 * gl_InstanceIndex, so the three instances land in the left, centre,
 * and right thirds of the frame. The host reads the image back and
 * asserts all three thirds contain rendered pixels — proving the
 * device VS resolved a distinct gl_InstanceIndex per instance. A
 * broken/ignored InstanceIndex would draw all three instances
 * overlapping in the centre, leaving the left and right thirds empty
 * (test fails).
 *
 * Run against lavapipe with GALLIUM_DRIVER=vortexpipe: it exercises
 * vkCmdDraw with instanceCount=3 on the vortexpipe device path.
 */
...
{
   const char *vs_path = (argc > 1) ? argv[1] : "instanced.vert.spv";
   const char *fs_path = (argc > 2) ? argv[2] : "instanced.frag.spv";

   /* --- instance --------------------------------------------------- */
   VkApplicationInfo app = {
      .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "vortexpipe-instanced",
   ...
   /* gl_VertexIndex / gl_InstanceIndex-driven: no vertex buffers. */
   ...
   /* vertexCount=3, instanceCount=INSTANCES, firstVertex=0, firstInstance=0 */
   vkCmdDraw(cmd, 3, INSTANCES, 0, 0);
   ...
   /* --- read back + verify ---------------------------------------- *
    * The three instances land in the left / centre / right thirds of
    * the frame; every third must carry rendered pixels. An ignored
    * gl_InstanceIndex would collapse all three onto the centre, so the
    * left and right bands would be empty. */
   uint8_t *px;
   CHECK(vkMapMemory(dev, bmem, 0, bytes, 0, (void **)&px));

   const uint32_t L = WIDTH / 3;            /* [0, L)   left third   */
   const uint32_t R = 2u * WIDTH / 3;       /* [R, W)   right third  */
   unsigned band[3] = { 0, 0, 0 };          /* left, centre, right   */
   unsigned colored = 0;
   ...
   /* each instance is a small triangle (~a few hundred px); require a
    * meaningful count in every third so a single lit band cannot pass. */
   const unsigned MIN_PER_BAND = 20u;
   bool all_bands = band[0] > MIN_PER_BAND &&
                    band[1] > MIN_PER_BAND &&
                    band[2] > MIN_PER_BAND;
   bool coverage_ok = colored > 100u && colored < 6000u;
   ...
   if (!all_bands || !coverage_ok) {
      printf("FAILED (bands L=%u C=%u R=%u colored=%u) — %u instances expected "
             "across all three thirds\n",
             band[0], band[1], band[2], colored, INSTANCES);
      return 1;
   }
   printf("PASSED (%u instances rendered distinctly: L=%u C=%u R=%u, %u/%u px)\n",
          INSTANCES, band[0], band[1], band[2], colored, WIDTH * HEIGHT);
   return 0;
}
